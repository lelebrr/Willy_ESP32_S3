...
void WillyTheme::removeTheme(void) {
  themeInfo t;
  theme = t;
}
FS *WillyTheme::themeFS(void) {
  if (theme.fs == 1)
    return &LittleFS;
  else if (theme.fs == 2)
    return &SD;
...
bool WillyTheme::openThemeFile(FS *fs, String filepath,
                               bool overwriteConfigSettings) {

  Serial.printf("[THEME] Tentando abrir tema: fs=%p, filepath='%s'\n", fs,
                filepath.c_str());
  if (fs == nullptr) {
    Serial.println("[THEME] FS é nullptr, retornando true");
    return true;
  }
  if (!fs->exists(filepath)) {
    Serial.printf("[THEME] Arquivo não existe: %s\n", filepath.c_str());
    return false;
  }
  File file;
  ...
  String baseThemePath =
      themePath.substring(0, themePath.lastIndexOf('/')) + "/";
    ...
    if (!_th[entry.key].isNull()) {
      ...
      if (fs->exists(path)) {
        *entry.flag = true;
        ...
        // Pre-cache PNGs into BIN files to avoid runtime decoding and
        // allocations
        if (path.endsWith(".png") || path.endsWith(".PNG")) {
          preparePngBin(*fs, path);
        }
      } else {
  ...
  if (!_th["gifDuration"].isNull()) {
    theme.gifDuration = _th["gifDuration"].as<int>();
  }

  if (overwriteConfigSettings) {
    uint16_t _priColor = willyConfig.priColor;
    uint16_t _secColor = willyConfig.secColor;
    uint16_t _bgColor = willyConfig.bgColor;
...
#ifdef HAS_RGB_LED
    if (!_th["ledBright"].isNull()) {
      willyConfig.ledBright = _th["ledBright"].as<int>();
    }
    if (!_th["ledColor"].isNull()) {
      willyConfig.ledColor = strtoul(_th["ledColor"], nullptr, 16);
    }
    if (!_th["ledEffect"].isNull()) {
      willyConfig.ledEffect = _th["ledEffect"].as<int>();
    }
    if (!_th["ledEffectSpeed"].isNull()) {
      willyConfig.ledEffectSpeed = _th["ledEffectSpeed"].as<int>();
    }
    if (!_th["ledEffectDirection"].isNull()) {
      willyConfig.ledEffectDirection = _th["ledEffectDirection"].as<int>();
    }
    ledSetup();
#endif
  }
...
bool WillyTheme::validateImgFile(FS *fs, String filepath) {
  // Think of a way to check if the images are at maximum height of tftHeight
  // this size is the maximun value to be shown on screen without overlapping
  // the status bar.
  return true;
}

void WillyTheme::_setUiColor(uint16_t primary, uint16_t *secondary,
                             uint16_t *background) {
  priColor = primary;
