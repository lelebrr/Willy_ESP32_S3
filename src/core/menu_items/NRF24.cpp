#include "NRF24.h"
#include "core/display.h"
#include "core/utils.h"
#include "modules/NRF24/nrf_common.h"
#include "modules/NRF24/nrf_jammer.h"
#include "modules/NRF24/nrf_spectrum.h"
#include "modules/NRF24/nrf_mousejack.h"
#include "modules/NRF24/nrf_fuzzer.h"
#include "modules/NRF24/nrf_fingerprint.h"
#include "modules/NRF24/nrf_targeted_jammer.h"
#include "modules/NRF24/nrf_replay.h"
#include "modules/NRF24/nrf_impersonation.h"
#include "modules/NRF24/nrf_unifying.h"
#include "modules/NRF24/nrf_stealth_exfil.h"
#include "modules/NRF24/nrf_mass_pairing.h"
#include "modules/NRF24/nrf_geolocator.h"

void NRF24Menu::optionsMenu() {
    options.clear();
    options.push_back(Option("Informacao", nrf_common::nrf_info));

    if (willyConfigPins.NRF24_bus.mosi == willyConfigPins.SDCARD_bus.mosi &&
        willyConfigPins.NRF24_bus.mosi != GPIO_NUM_NC)
        options.push_back(Option("Espectro", [=]() { nrf_spectrum(&sdcardSPI); }));
#if TFT_MOSI > 0 // Display doesn't use SPI bus
    else if (willyConfigPins.NRF24_bus.mosi == (gpio_num_t)TFT_MOSI)
        options.push_back(Option("Espectro", [=]() { nrf_spectrum(&tft.getSPIinstance()); }));
#endif
    else options.push_back(Option("Espectro", [=]() { nrf_spectrum(&SPI); }));

    options.push_back(Option("Jammer NRF", nrf_jammer));
    options.push_back(Option("Jammer Canal", nrf_channel_jammer));
    options.push_back(Option("Salto de Canal", nrf_channel_hopper));

    options.push_back(Option("Ataques NRF24", [this]() { attacksMenu(); }));

#if defined(ARDUINO_M5STICK_C_PLUS) || defined(ARDUINO_M5STICK_C_PLUS2)
    options.push_back(Option("Config pinos", [this]() { configMenu(); }));
#endif

    addOptionToMainMenu();

    loopOptions(options, MENU_TYPE_SUBMENU, "NRF24");
}

void NRF24Menu::attacksMenu() {
    options = {
        {"MouseJack Inject", nrf_mousejack_injector},
        {"Smart Fuzzer", nrf_smart_fuzzer},
        {"Fingerprint", nrf_device_fingerprint},
        {"Target Jammer", nrf_targeted_jammer},
        {"Replay Attack", nrf_keystroke_replay},
        {"Impersonation MITM", nrf_device_impersonation},
        {"Unifying Exploit", nrf_unifying_exploit},
        {"Stealth Exfil", nrf_stealth_exfil},
        {"Mass Pairing Brute", nrf_mass_pairing},
        {"Geo-Locator", nrf_signal_geolocator},
        {"Voltar", [this]() { optionsMenu(); }}
    };

    loopOptions(options, MENU_TYPE_SUBMENU, "Ataques NRF24");
}

void NRF24Menu::configMenu() {
    uint8_t opt = 0;
    int idx = 0;
    if (willyConfigPins.NRF24_bus.mosi == (gpio_num_t)SDCARD_MOSI) idx = 1;
    options = {
        {"NRF24 (Legado)",     [&]() { opt = 1; }         },
        {"NRF24 (SPI Comp.)", [&]() { opt = 2; }         },
        {"Voltar",               [this]() { optionsMenu(); }},
    };

    loopOptions(options, MENU_TYPE_SUBMENU, "Config RF", idx);
    if (opt == 1) {
        willyConfigPins.setNrf24Pins(
            {(gpio_num_t)NRF24_SCK_PIN,
             (gpio_num_t)NRF24_MISO_PIN,
             (gpio_num_t)NRF24_MOSI_PIN,
             (gpio_num_t)NRF24_SS_PIN,
             (gpio_num_t)NRF24_CE_PIN,
             GPIO_NUM_NC}
        );
        willyConfigPins.setCC1101Pins(
            {(gpio_num_t)NRF24_SCK_PIN,
             (gpio_num_t)NRF24_MISO_PIN,
             (gpio_num_t)NRF24_MOSI_PIN,
             (gpio_num_t)NRF24_SS_PIN,
             (gpio_num_t)NRF24_CE_PIN,
             GPIO_NUM_NC}
        );
    }
#if CONFIG_SOC_GPIO_OUT_RANGE_MAX > 30
    if (opt == 2) {
        willyConfigPins.setNrf24Pins(
            {(gpio_num_t)SDCARD_SCK,
             (gpio_num_t)SDCARD_MISO,
             (gpio_num_t)SDCARD_MOSI,
             GPIO_NUM_33,
             GPIO_NUM_32,
             GPIO_NUM_NC}
        );
        willyConfigPins.setCC1101Pins(
            {(gpio_num_t)SDCARD_SCK,
             (gpio_num_t)SDCARD_MISO,
             (gpio_num_t)SDCARD_MOSI,
             GPIO_NUM_33,
             GPIO_NUM_32,
             GPIO_NUM_NC}
        );
    }
#endif
}

void NRF24Menu::drawIcon(float scale) {
    clearIconArea();
    int iconW = scale * 80;
    int iconH = scale * 60;

    if (iconW % 2 != 0) iconW++;
    if (iconH % 2 != 0) iconH++;

    int caseW = 3 * iconW / 4;
    int caseH = 2 * iconH / 3;
    int caseX = iconCenterX - iconW / 2;
    int caseY = iconCenterY - iconH / 6;

    int antW = iconW / 8;
    int connR = iconH / 20;

    // Case
    tft.drawRect(caseX, caseY, caseW, caseH, willyConfig.priColor);

    // Antenna
    tft.fillRect(caseX + caseW, caseY + caseH / 2 - antW / 2, antW, antW, willyConfig.priColor);
    tft.fillRoundRect(
        caseX + caseW + antW,
        caseY + caseH - iconH,
        antW,
        iconH - caseH / 2 + antW / 2,
        antW / 2,
        willyConfig.priColor
    );

    // Connectors
    tft.fillCircle(caseX + caseW / 6, caseY + 1 * caseH / 5, connR, willyConfig.priColor);
    tft.fillCircle(caseX + caseW / 6, caseY + 2 * caseH / 5, connR, willyConfig.priColor);
    tft.fillCircle(caseX + caseW / 6, caseY + 3 * caseH / 5, connR, willyConfig.priColor);
    tft.fillCircle(caseX + caseW / 6, caseY + 4 * caseH / 5, connR, willyConfig.priColor);

    tft.fillCircle(caseX + caseW / 3, caseY + 1 * caseH / 5, connR, willyConfig.priColor);
    tft.fillCircle(caseX + caseW / 3, caseY + 2 * caseH / 5, connR, willyConfig.priColor);
    tft.fillCircle(caseX + caseW / 3, caseY + 3 * caseH / 5, connR, willyConfig.priColor);
    tft.fillCircle(caseX + caseW / 3, caseY + 4 * caseH / 5, connR, willyConfig.priColor);

    // Chip
    tft.fillRect(
        caseX + caseW - 2 * antW - connR, caseY + caseH / 2 - antW / 2, antW, antW, willyConfig.priColor
    );
}
