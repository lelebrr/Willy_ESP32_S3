...
//  3. Visit 192.168.4.1 in your browser to access the web interface for shell
//  executing.

// 32bit:
// https://github.com/9dl/Willy-C2/releases/download/v1.0/WillyC2_windows_386.exe
// 64bit:
// https://github.com/9dl/Willy-C2/releases/download/v1.0/WillyC2_windows_amd64.exe
...
void WifiMenu::optionsMenu() {
  ...
  if (WiFi.getMode() != WIFI_MODE_NULL)
    options.push_back(Option("Desligar WiFi", wifiDisconnect));
  ...
  options.push_back(Option("Ataques WiFi", [=]() {
    if (WiFi.getMode() == WIFI_AP || WiFi.getMode() == WIFI_AP_STA) {
      if (displayMessage(
              "Atacar em modo AP pode\ndesconectar clientes.\nContinuar?",
              "Não", nullptr, "Sim", TFT_YELLOW) != 2)
        return;
    }
    wifi_atk_menu();
  }));
...
#ifndef LITE_VERSION
  options.push_back(Option("Ouvir TCP", [=]() { listenTcpPort(0); }));
...
void WifiMenu::configMenu() {
  ...
  wifiOptions.push_back(
      {"Config Evil Wifi", [this]() {
         ...
         evilOptions.push_back(
             {"Mostrar endpoints", setEvilAllowEndpointDisplay});
...

  {
    ...
    String hidden__wifi_option =
        String("Redes Ocultas: ") + (showHiddenNetworks ? "LIG" : "DES");
    ...
    Option opt(hidden__wifi_option.c_str(), [this]() {
      ...
      displayInfo(String("Redes Ocultas: ") +
                      (showHiddenNetworks ? "LIG" : "DES"),
                  true);
      configMenu();
    });
...
void WifiMenu::drawIcon(float scale) {
  ...
  tft.fillCircle(iconCenterX, iconCenterY + deltaY, radius,
                 willyConfig.priColor);
  tft.drawArc(iconCenterX, iconCenterY + deltaY, deltaY + radius, deltaY, 130,
              230, willyConfig.priColor, willyConfig.bgColor);
  tft.drawArc(iconCenterX, iconCenterY + deltaY, 2 * deltaY + radius,
              2 * deltaY, 130, 230, willyConfig.priColor, willyConfig.bgColor);
}
