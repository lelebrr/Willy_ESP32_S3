...
String listFiles(FS &fs, String folder) {
  ...
  while (true) {
    ...
    if (esp_get_free_heap_size() >
        (String("Fo:" + nameOnly + ":0\n").length()) + 1024) {
      if (isDir) {
        ...
        if (file) {
          returnText +=
              "Fi:" + nameOnly + ":" + humanReadableSize(file.size()) + "\n";
          file.close();
        }
      }
    } else
...
bool checkUserWebAuth(AsyncWebServerRequest *request,
                      bool onFailureReturnLoginPage = false) {
  if (request->hasHeader("Cookie")) {
    ...
    if (idx != -1) {
      ...
      if (willyConfig.isValidWebUISession(token)) {
        return true;
      }
    }
  }
  if (onFailureReturnLoginPage) {
    serveWebUIFile(request, "login.html", "text/html", true, login_html,
                   login_html_size);
  } else {
...
void createDirRecursive(String path, FS fs) {
  ...
  while (startIndex < (size_t)path.length()) {
    ...
    if ((size_t)endIndex < (size_t)path.length()) {
      currentPath += "/";
    }
...
void handleUpload(AsyncWebServerRequest *request, String filename, size_t index,
  ...
  if (checkUserWebAuth(request)) {
    ...
    if (!index) {
      ...
      if (dirPath.length() > 0) {
        createDirRecursive(dirPath, _webFS);
      }
    RETRY:
      request->_tempFile = _webFS.open(uploadFolder + "/" + filename, "w");
      if (!request->_tempFile) {
        // Serial.println("Failed to open file for writing: " + uploadFolder +
        // "/" + filename);
        goto RETRY;
      }
    }

    static String encrypted_accumulator = "";

    if (len) {
      if (request->hasArg("password")) {
        if (index == 0)
          encrypted_accumulator = ""; // reset for new file
        encrypted_accumulator += String((char *)data).substring(0, len);
      } else {
        if (request->_tempFile)
    ...
    if (final) {
      if (request->hasArg("password")) {
        ...
        String cyphertxt = encryptString(encrypted_accumulator, enc_password);
        if (cyphertxt != "" && request->_tempFile) {
          request->_tempFile.write((const uint8_t *)cyphertxt.c_str(),
                                   cyphertxt.length());
        }
        encrypted_accumulator = ""; // clear memory
      }
...
void drawWebUiScreen(bool mode_ap) {
  ...
  if (mode_ap) {
    setTftDisplay(0, 0, willyConfig.bgColor, FM);
  ...
  tft.println("Usr: " + String(willyConfig.webUI.user));
  tft.setCursor(7, tft.getCursorY());
  tft.println("Pwd: " + String(willyConfig.webUI.pwd));
  tft.setCursor(7, tft.getCursorY());
  tft.setTextColor(TFT_RED);
  tft.setTextSize(FP);
  tft.drawCentreString("press Esc to stop", tftWidth / 2,
                       tftHeight - 2 * LH * FP, 1);
...
**********************************************************************/
void serveWebUIFile(AsyncWebServerRequest *request, String filename,
                    const char *contentType) {
  serveWebUIFile(request, filename, contentType, false, nullptr, 0);
}
void serveWebUIFile(AsyncWebServerRequest *request, String filename,
                    const char *contentType, bool gzip,
                    const uint8_t *originaFile, uint32_t originalFileSize) {
  AsyncWebServerResponse *response = nullptr;
  FS *fs = NULL;
  if (setupSdCard()) {
    if (SD.exists("/WillyWebUI/" + filename))
      fs = &SD;
  ...
  if (fs) {
    response =
        request->beginResponse(*fs, "/WillyWebUI/" + filename, contentType);
  } else {
    if (filename == "theme.css") {
      String css = ":root{--color:" + color565ToWebHex(willyConfig.priColor) +
                   ";--sec-color:" + color565ToWebHex(willyConfig.secColor) +
                   ";--background:" + color565ToWebHex(willyConfig.bgColor) +
                   ";}";
      AsyncWebServerResponse *themeResponse =
          request->beginResponse(200, "text/css", css);
      request->send(themeResponse);
      return;
    }
    response = request->beginResponse(200, String(contentType), originaFile,
                                      originalFileSize);
    if (gzip) {
      if (!response->addHeader("Content-Encoding", "gzip"))
        log_e("Failed to add gzip header");
    }
  }
...
static bool startMdnsResponder() {
  constexpr size_t kMinInternalHeap =
      20 * 1024; // bytes reserved for mDNS buffers
  size_t freeInternalHeap = heap_caps_get_free_size(MALLOC_CAP_INTERNAL);
  if (freeInternalHeap < kMinInternalHeap) {
    log_e("Skipping mDNS responder. Only %lu bytes of internal heap available "
          "(need %lu).\n",
...
void configureWebServer() {
  ...
  server->on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (checkUserWebAuth(request, true)) {
      serveWebUIFile(request, "index.html", "text/html", true, index_html,
                     index_html_size);
    }
  });
  ...
  server->on("/login", HTTP_POST, [](AsyncWebServerRequest *request) {
    if (request->hasParam("username", true) &&
      ...
      if (username == willyConfig.webUI.user &&
          password == willyConfig.webUI.pwd) {
        String token = generateToken();
        AsyncWebServerResponse *response = request->beginResponse(302);
        response->addHeader("Location", "/");
        response->addHeader("Set-Cookie",
                            "WILLYSESSION=" + token + "; Path=/; HttpOnly");
        request->send(response);
        willyConfig.addWebUISession(token);
  ...
  server->on("/logout", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (request->hasHeader("Cookie")) {
      ...
      if (idx != -1) {
        ...
        String token = c.substring(start, end);
        willyConfig.removeWebUISession(token);
      }
    }
    AsyncWebServerResponse *response = request->beginResponse(302);
    ...
    response->addHeader(
        "Set-Cookie",
        "WILLYSESSION=0; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT");
    request->send(response);
  });
  ...
  server->on("/index.css", HTTP_GET, [](AsyncWebServerRequest *request) {
    serveWebUIFile(request, "index.css", "text/css", true, index_css,
                   index_css_size);
  });
  server->on("/index.js", HTTP_GET, [](AsyncWebServerRequest *request) {
    serveWebUIFile(request, "index.js", "text/javascript", true, index_js,
                   index_js_size);
  });
  ...
  server->on("/systeminfo", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (checkUserWebAuth(request)) {
      ...
      sprintf(
          ...
          "WILLY_VERSION", WILLY_VERSION, "free",
  ...
  server->on("/getscreen", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (checkUserWebAuth(request)) {
      ...
      if (!screenBinBuffer) {
        ...
        if (psramFound())
          screenBinBuffer = static_cast<uint8_t *>(ps_malloc(desiredSize));
        if (!screenBinBuffer)
          screenBinBuffer = static_cast<uint8_t *>(malloc(desiredSize));
        if (!screenBinBuffer) {
          request->send(503, "text/plain",
                        "Insufficient memory for screen buffer");
          return;
        }
      ...
      request->send(200, "application/octet-stream",
                    (const uint8_t *)screenBinBuffer, binSize);
    }
  });
  ...
  server->on("/cm", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    if (request->hasArg("cmnd")) {
      ...
      if (cmnd.startsWith("nav")) {
        ...
        if (cmnd.startsWith("nav sel"))
          var = &SelPress;
        if (cmnd.startsWith("nav esc"))
          var = &EscPress;
        if (cmnd.startsWith("nav up"))
          var = &UpPress;
        if (cmnd.startsWith("nav down"))
          var = &DownPress;
        if (cmnd.startsWith("nav next"))
          var = &NextPress;
        if (cmnd.startsWith("nav prev"))
          var = &PrevPress;
        request->send(200, "text/plain", "command " + cmnd + " success");
        int time;
        if (cmnd.endsWith("0"))
          time = cmnd.substring(cmnd.lastIndexOf(' ')).toInt();
        else
          time = 10;
      ...
      request->send(400, "text/plain",
                    "http request missing required arg: cmnd");
    }
  });
      ...
      if (request->hasArg("name") && request->hasArg("action")) {
        ...
        if (!fs->exists(fileName)) {
          ...
          if (strcmp(fileAction.c_str(), "download") == 0) {
            ...
            if (extension == "jpg")
              extension = "jpeg"; // www.rfc-editor.org/rfc/rfc2046.html
  ...
  server->on("/wifi", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (checkUserWebAuth(request)) {
      if (request->hasArg("usr") && request->hasArg("pwd")) {
        ...
        willyConfig.setWebUICreds(usr, pwd);
        request->send(200, "text/plain",
                      "User: " + String(usr) +
