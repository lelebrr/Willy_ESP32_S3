...
String ScrollableTextArea::getLine(size_t lineNumber) {
    if (lineNumber >= linesBuffer.size() || !linesBuffer[lineNumber]) return "";
...
void ScrollableTextArea::fromString(const String &text) {
    ...
    while ((size_t)endIdx < text.length()) {
        if (text[endIdx] == '\n') {
    ...
    if ((size_t)startIdx < text.length()) { addLine(text.substring(startIdx, endIdx)); }
}
...
void ScrollableTextArea::addLine(const String &text) {
    if (text.isEmpty()) {
        ...
        linesBuffer.push_back(emptyStr);
        return;
    }
    ...
    while (start < text.length()) {
        ...
        char *ptr = (char *)(psramFound() ? ps_malloc(buff.length() + 1) : malloc(buff.length() + 1));
        if (ptr) {
            strcpy(ptr, buff.c_str());
            linesBuffer.push_back(ptr);
        }
        firstLine = false;
    }
...
void ScrollableTextArea::draw(bool force) {
    ...
    while (yOffset < tmpHeight && lines < _maxVisibleLines && idx < linesBuffer.size()) {
        if (linesBuffer[idx]) {
            _scrollBuffer.drawString(String(linesBuffer[idx]), 0 + _startX, _startY + yOffset);
        }
        yOffset += _pixelsPerLine;
        lines++;
        idx++;
