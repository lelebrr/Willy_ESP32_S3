...
const char *LEVEL_STRINGS[] = {"TRACE", "DEBUG", "INFO", "NOTICE",
                               "WARN",  "ERROR", "CRIT", "OFF"};

// Strings de componente
const char *COMPONENT_STRINGS[] = {"SYSTEM", "GPS",   "IR",   "WIFI", "BLE",
                                   "CC1101", "NRF24", "NFC",  "RF",   "BADUSB",
                                   "ETH",    "SD",    "DISP", "PWR",  "WEB",
                                   "INTERP", "LORA",  "FM",   "OTHER"};
...
bool WillyLogger::begin() {
  ...
  if (_config.logToSD) {
    ...
    if (!_logFile) {
      ...
      _logFile.println(F("Timestamp,Level,Component,Message,Value1,Value2,"
                         "Value3,HeapFree,ErrorCode"));
      _logFile.flush();
    }
  }
...
void WillyLogger::log(WillyLogLevel level, WillyLogComponent component,
  ...
  if (!_config.enabled || level < _config.minLevel)
    return;
  ...
  _totalEntries++;
  if (level == WILLY_LOG_LEVEL_ERROR)
    _errorCount++;
  if (level == WILLY_LOG_LEVEL_WARNING)
    _warningCount++;
...
void WillyLogger::trace(WillyLogComponent comp, const char *msg, int32_t v1,
                        int32_t v2, int32_t v3) {
  log(WILLY_LOG_LEVEL_TRACE, comp, msg, v1, v2, v3);
}

void WillyLogger::debug(WillyLogComponent comp, const char *msg, int32_t v1,
                        int32_t v2, int32_t v3) {
  log(WILLY_LOG_LEVEL_DEBUG, comp, msg, v1, v2, v3);
}

void WillyLogger::info(WillyLogComponent comp, const char *msg, int32_t v1,
                       int32_t v2, int32_t v3) {
  log(WILLY_LOG_LEVEL_INFO, comp, msg, v1, v2, v3);
}

void WillyLogger::notice(WillyLogComponent comp, const char *msg, int32_t v1,
                         int32_t v2, int32_t v3) {
  log(WILLY_LOG_LEVEL_NOTICE, comp, msg, v1, v2, v3);
}

void WillyLogger::warning(WillyLogComponent comp, const char *msg, int32_t v1,
                          int32_t v2, int32_t v3) {
  log(WILLY_LOG_LEVEL_WARNING, comp, msg, v1, v2, v3);
}

void WillyLogger::error(WillyLogComponent comp, const char *msg, int32_t v1,
                        int32_t v2, int32_t v3, uint16_t code) {
  log(WILLY_LOG_LEVEL_ERROR, comp, msg, v1, v2, v3, code);
}

void WillyLogger::critical(WillyLogComponent comp, const char *msg, int32_t v1,
                           int32_t v2, int32_t v3, uint16_t code) {
...
void WillyLogger::logGPS(float lat, float lon, float alt, int satellites,
                         uint32_t age, bool valid) {
  if (valid) {
    logf(WILLY_LOG_LEVEL_INFO, COMP_GPS,
         "GPS Fix: lat=%.6f, lon=%.6f, alt=%.1fm, sats=%d, age=%lums", lat, lon,
         alt, satellites, age);
  } else {
    logf(WILLY_LOG_LEVEL_WARNING, COMP_GPS, "GPS sem fix: sats=%d, age=%lums",
         satellites, age);
  }
}

void WillyLogger::logIR(uint64_t code, uint16_t protocol, uint16_t bits,
                        bool raw) {
  if (raw) {
    logf(WILLY_LOG_LEVEL_INFO, COMP_IR, "IR Raw: code=0x%llX, bits=%d", code,
         bits);
  } else {
    logf(WILLY_LOG_LEVEL_INFO, COMP_IR,
         "IR Code: 0x%llX, protocol=%d (%s), bits=%d", code, protocol,
         getProtocolString(protocol), bits);
  }
}

void WillyLogger::logWiFiScan(int apCount, int rssi, const char *ssid) {
  if (ssid) {
    logf(WILLY_LOG_LEVEL_INFO, COMP_WIFI,
         "WiFi Scan: %d APs, RSSI=%d dBm, SSID=%s", apCount, rssi, ssid);
  } else {
    logf(WILLY_LOG_LEVEL_INFO, COMP_WIFI,
         "WiFi Scan: %d APs encontrados, RSSI médio=%d dBm", apCount, rssi);
  }
}

void WillyLogger::logBLEScan(int deviceCount, const char *name) {
  if (name) {
    logf(WILLY_LOG_LEVEL_INFO, COMP_BLE, "BLE Scan: %d devices, found=%s",
         deviceCount, name);
  } else {
    logf(WILLY_LOG_LEVEL_INFO, COMP_BLE, "BLE Scan: %d devices encontrados",
         deviceCount);
  }
}

void WillyLogger::logCC1101(float freq, int rssi, const char *mode) {
  logf(WILLY_LOG_LEVEL_INFO, COMP_CC1101,
       "CC1101: freq=%.3f MHz, RSSI=%d dBm, mode=%s", freq, rssi, mode);
}

void WillyLogger::logNRF24(int channel, int packetCount, const char *mode) {
  logf(WILLY_LOG_LEVEL_INFO, COMP_NRF24, "NRF24: ch=%d, packets=%d, mode=%s",
       channel, packetCount, mode);
}
...
void WillyLogger::logRF(uint64_t code, float freq, int protocol,
                        const char *raw) {
  if (raw) {
    logf(WILLY_LOG_LEVEL_INFO, COMP_RF, "RF Raw: freq=%.3f MHz, data=%s", freq,
         raw);
  } else {
    logf(WILLY_LOG_LEVEL_INFO, COMP_RF,
         "RF Code: 0x%llX, freq=%.3f MHz, protocol=%d", code, freq, protocol);
  }
}

void WillyLogger::logError(WillyLogComponent comp, const char *context,
                           int errorCode, const char *details) {
  if (details) {
    logf(WILLY_LOG_LEVEL_ERROR, comp, "ERRO [%s]: code=%d, %s", context,
         errorCode, details);
  } else {
...
void WillyLogger::logSystemStatus() {
  logf(WILLY_LOG_LEVEL_INFO, COMP_SYSTEM,
       "Status: heap=%d, freePSRAM=%d, millis=%lu", ESP.getFreeHeap(),
       ESP.getFreePsram(), millis());
}

void WillyLogger::logModuleInit(const char *moduleName, bool success,
                                const char *details) {
  if (success) {
...
bool WillyLogger::rotateLog() {
  ...
  // Cabeçalho
  _logFile.println(F("Timestamp,Level,Component,Message,Value1,Value2,Value3,"
                     "HeapFree,ErrorCode"));
  _logFile.flush();
...
void WillyLogger::getStats(uint32_t &totalEntries, uint32_t &errorCount,
...
String WillyLogger::generateLogFilename() {
  ...
  if (getLocalTime(&timeinfo, 100)) {
    snprintf(filename, sizeof(filename),
             "%s/willy_%04d%02d%02d_%02d%02d%02d.csv",
             _config.logDirectory.c_str(), timeinfo.tm_year + 1900,
...
const char *getProtocolString(uint16_t protocol) {
  static const char *protocols[] = {
      "UNKNOWN",       "RC5",          "RC6",        "NEC",
      "SONY",          "PANASONIC",    "JVC",        "SAMSUNG",
      "WHYNTER",       "AIWA_RC_T501", "LG",         "SANYO",
      "MITSUBISHI",    "DISH",         "SHARP",      "COOLIX",
      "DAIKIN",        "DENON",        "KELVINATOR", "SHERWOOD",
      "MITSUBISHI_AC", "RCMM",         "GREE",       "PRONTO",
      "NEC_LIKE"};
