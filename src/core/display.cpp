...
void displayScrollingText(const String &text, Opt_Coord &coord) {
  ...
  if (len < (int)coord.size) {
    // Text fits within limit, no scrolling needed
    return;
  } else if (millis() > _lastmillis + 200) {
    String scrollingPart = displayText.substring(
        i, i + (coord.size - 1)); // Display charLimit characters at a time
    tft.fillRect(coord.x, coord.y, (coord.size - 1) * LW * tft.getTextSize(),
                 LH * tft.getTextSize(),
                 willyConfig.bgColor); // Clear display area
...
void displayRedStripe(String text, uint16_t fgcolor, uint16_t bgcolor) {
  // detect if not running in interactive mode -> show nothing onscreen and
  // return immediately if (server || isSleeping || isScreenOff) return; //
  // webui is running

  int size;
  if (fgcolor == bgcolor && fgcolor == TFT_WHITE)
    fgcolor = TFT_BLACK;
  if (text.length() * LW * FM < (uint32_t)(tftWidth - 2 * FM * LW))
    size = FM;
  else
    size = FP;
  tft.drawPixel(0, 0, 0);
...
void drawButton(int32_t x, int32_t y, int32_t w, int32_t h, uint32_t color,
...
int8_t displayMessage(const char *message, const char *leftButton,
  ...
  int8_t totalButtons =
      (leftButton ? 1 : 0) + (centerButton ? 1 : 0) + (rightButton ? 1 : 0);
  ...
  while (true) {
    ...
    if (redraw) {
      if (leftButton) {
        drawButton(5, buttonY, buttonWidth, buttonHeight, color, leftButton,
                   selected == index);
        index++;
      }

      if (centerButton) {
        drawButton(tftWidth / 3 + 5, buttonY, buttonWidth, buttonHeight, color,
                   centerButton, selected == index);
        index++;
      }

      if (rightButton) {
        drawButton(tftWidth * 2 / 3 + 5, buttonY, buttonWidth, buttonHeight,
                   color, rightButton, selected == index);
      }
      redraw = false;
    }
...
#endif
  delay(200);
  while (waitKeyPress && !check(AnyKeyPress))
...
void displayTextLine(String txt, bool waitKeyPress) {
  // todo: add newlines to txt if too long
  displayRedStripe(txt, getComplementaryColor2(willyConfig.priColor),
...
void padprintln(const char str[], int16_t padx) {
  ...
  // automatically split into multiple lines
  while (!(buff = String(str).substring(start, start + _maxCharsInLine))
              .isEmpty()) {
    tft.setCursor(padx * BORDER_PAD_X, tft.getCursorY());
    tft.println(buff);
    start += buff.length();
...
int loopOptions(std::vector<Option> &options, uint8_t menuType,
  ...
  if (options.size() > static_cast<size_t>(MAX_MENU_SIZE)) {
    menuSize = MAX_MENU_SIZE;
  }
  if (index > 0)
    tft.fillRoundRect(
        tftWidth * 0.10, tftHeight / 2 - menuSize * (FM * 8 + 4) / 2 - 5,
        tftWidth * 0.8, (FM * 8 + 4) * menuSize + 10, 5, willyConfig.bgColor);
  if (static_cast<size_t>(index) >= options.size())
    index = 0;
  bool firstRender = true;
  drawMainBorder();
  while (1) {
    // Check for shutdown before drawing menu to avoid drawing a black bar on
    // the screen
    if (exit)
      break;
    if (menuType == MENU_TYPE_MAIN) {
      checkReboot();
      if (devModeCounter >= 5 && !willyConfig.devMode) {
        willyConfig.setDevMode(true);
    ...
    if (redraw) {
      ...
      if (options[index].hover)
        renderedByLambda =
            options[index].hover(options[index].hoverPointer, true);

      if (!renderedByLambda) {
        if (menuType == MENU_TYPE_SUBMENU)
          drawSubmenu(index, options, subText);
        else
          coord = drawOptions(index, options, willyConfig.priColor,
                              willyConfig.secColor, willyConfig.bgColor,
                              firstRender);
      }
      firstRender = false;
      redraw = false;
...
#ifdef HAS_KEYBOARD
    checkShortcutPress(); // shortctus to quickly start apps without navigating
                          // the menus
#endif
    ...
    // PrevPress condition is a StickCPlus workaround, as it uses the same
    // button for Prev and Esc Same happens to Core and some other boards
    ...
    if (PrevPress || check(UpPress)) {
      ...
      while (PrevPress && menuType != MENU_TYPE_MAIN) {
        if (millis() - _tmp > 200)
          tft.drawArc(tftWidth / 2, tftHeight / 2, 25, 15, 0,
    ...
    if (check(NextPress) || check(DownPress)) {
      index++;
      if ((size_t)(index + 1) > options.size()) {
    ...
    forceMenuOption is set by a SerialCommand to force a selection within the
    menu
    */
    ...
    if (interpreter_state > 0 && !interpreter) {
      break;
    }
  }
  return index;
...
void progressHandler(int progress, size_t total, String message) {
  ...
  if (barWidth < 3) {
    tft.fillRect(6, 27, tftWidth - 12, tftHeight - 33, willyConfig.bgColor);
    tft.drawRect(18, tftHeight - 47, tftWidth - 36, 17, willyConfig.priColor);
    displayRedStripe(message, TFT_WHITE, willyConfig.priColor);
  }
  tft.fillRect(20, tftHeight - 45, barWidth, 13, willyConfig.priColor);
}
...
Opt_Coord drawOptions(int index, std::vector<Option> &options, uint16_t fgcolor,
                      uint16_t selcolor, uint16_t bgcolor, bool firstRender) {
  Opt_Coord coord;
  int menuSize = options.size();
  if (options.size() > (size_t)MAX_MENU_SIZE) {
    menuSize = MAX_MENU_SIZE;
  }
  ...
  if (firstRender) {
    tft.fillRoundRect(tftWidth * 0.10, optionsTopY, tftWidth * 0.8,
                      (FM * 8 + 4) * menuSize + 10, 5, bgcolor);
    tft.drawRoundRect(tftWidth * 0.10,
                      tftHeight / 2 - menuSize * (FM * 8 + 4) / 2 - 5,
                      tftWidth * 0.8, (FM * 8 + 4) * menuSize + 10, 5, fgcolor);
  }

  tft.setTextColor(fgcolor, bgcolor);
  tft.setTextSize(FM);
  tft.setCursor(tftWidth * 0.10 + 5,
                tftHeight / 2 - menuSize * (FM * 8 + 4) / 2);
  ...
  for (i = 0; i < menuSize; i++) {
    if (i >= init) {
      if (options[i].selected)
        tft.setTextColor(selcolor, bgcolor); // if selected, change Text color
      else
        tft.setTextColor(fgcolor, bgcolor);
...
void drawSubmenu(int index, std::vector<Option> &options, const char *title) {
  ...
  tft.setTextSize(FP);
  tft.drawPixel(0, 0, 0);
  tft.fillRect(6, 30, tftWidth - 12, 8 * FP, willyConfig.bgColor);
  ...
  tft.setTextColor(willyConfig.secColor);
  tft.fillRect(6, middle_up, tftWidth - 12, 8 * FM, willyConfig.bgColor);
  tft.drawCentreString(firstOption, tftWidth / 2, middle_up, SMOOTH_FONT);

  // Selected item
  int selectedTextSize = options[index].label.length() <=
                                 static_cast<size_t>(tftWidth / (LW * FG) - 1)
                             ? FG
                             : FM;
  tft.setTextSize(selectedTextSize);
  tft.setTextColor(willyConfig.priColor);
  tft.fillRect(6, middle - FG * LH / 2 - 1, tftWidth - 12, FG * LH + 5,
               willyConfig.bgColor);
  tft.drawCentreString(options[index].label, tftWidth / 2,
                       middle - selectedTextSize * LH / 2, SMOOTH_FONT);
  tft.drawFastHLine(tftWidth / 2 - strlen(options[index].label.c_str()) *
                                       selectedTextSize * LW / 2,
                    middle + selectedTextSize * LH / 2 + 1,
                    strlen(options[index].label.c_str()) * selectedTextSize *
                        LW,
                    willyConfig.priColor);
  // Next Item
  ...
  tft.setTextSize(FM);
  tft.setTextColor(willyConfig.secColor);
  tft.fillRect(6, middle_down, tftWidth - 12, 8 * FM, willyConfig.bgColor);
  tft.drawCentreString(thirdOption, tftWidth / 2, middle_down, SMOOTH_FONT);

  tft.fillRect(tftWidth - 5, 0, 5, tftHeight, willyConfig.bgColor);
  tft.fillRect(tftWidth - 5, index * tftHeight / menuSize, 5,
               tftHeight / menuSize, willyConfig.priColor);
...
void drawStatusBar() {
  ...
  tft.setTextColor(TFT_DARKGREY, willyConfig.bgColor);
  tft.setTextSize(FP);
  tft.drawString("H:" + String(freeHeap) + "k C:" + String(cpuFreq) + "M", 10,
                 7);
  ...
  if (willyConfig.theme.border) {
    tft.drawRoundRect(5, 5, tftWidth - 10, tftHeight - 10, 5,
                      willyConfig.priColor);
    tft.drawLine(5, 25, tftWidth - 6, 25, willyConfig.priColor);
  }

  if (clock_set) {
    int clock_fontsize = 1; // Font size of the clock / WILLY + WILLY_VERSION
    setTftDisplay(12, 12, willyConfig.priColor, clock_fontsize,
                  willyConfig.bgColor);
    tft.fillRect(12, 12, 100, clock_fontsize * LH, willyConfig.bgColor);
#if defined(HAS_RTC)
    ...
    setTftDisplay(12, 12, willyConfig.priColor, 1, willyConfig.bgColor);
...
void drawMainBorder(bool clear) {
  ...
  // if(wifiConnected) {tft.print(timeStr);} else {tft.print("WILLY 1.0b");}
...
void printSubtitle(String subtitle, bool withLine) {
  ...
  if (withLine) {
    String line = "";
    for (byte i = 0; i < subtitle.length(); i++)
      line += "-";
...
void printFootnote(String text) {
  tft.setTextSize(FP);
  tft.drawRightString(text, tftWidth - BORDER_PAD_X,
                      tftHeight - BORDER_PAD_X - FP * LH, SMOOTH_FONT);
}

void printCenterFootnote(String text) {
  tft.fillRect(10, tftHeight - BORDER_PAD_X - FP * LH, tftWidth - 20, FP * LH,
               willyConfig.bgColor);
  tft.setTextSize(FP);
  tft.drawCentreString(text, tftWidth / 2, tftHeight - BORDER_PAD_X - FP * LH,
                       SMOOTH_FONT);
}
...
void drawWireguardStatus(int x, int y) {
  tft.fillRect(x, y, 20, 17, willyConfig.bgColor);
  if (isConnectedWireguard) {
    tft.drawRoundRect(11 + x, 0 + y, 8, 12, 5, TFT_GREEN);
    tft.fillRoundRect(10 + x, 8 + y, 10, 8, 0, TFT_GREEN);
  } else {
    tft.drawRoundRect(1 + x, 0 + y, 8, 12, 5, willyConfig.priColor);
    tft.fillRoundRect(0 + x, 8 + y, 10, 8, 0, willyConfig.bgColor);
    tft.fillRoundRect(6 + x, 8 + y, 10, 10, 0, willyConfig.priColor);
  }
}
  ...
  while (i < arraySize) {
    if (i >= start) {
      ...
      if (fileList[i].folder == true)
        tft.setTextColor(getColorVariation(willyConfig.priColor),
                         willyConfig.bgColor);
      else if (fileList[i].operation == true)
        tft.setTextColor(ALCOLOR, willyConfig.bgColor);
      else {
        tft.setTextColor(willyConfig.priColor, willyConfig.bgColor);
      }
      ...
      if (index == i) {
        ...
        coord.size = nchars;
        coord.fgcolor = fileList[i].folder
                            ? getColorVariation(willyConfig.priColor)
                            : willyConfig.priColor;
    ...
    i++;
    if (i == (start + MAX_ITEMS) || i == arraySize)
      break;
  }
  return coord;
}
...
void drawWifiSmall(int x, int y) {
  tft.fillRect(x, y, 16, 16, willyConfig.bgColor);
  tft.fillCircle(9 + x, 14 + y, 1, willyConfig.priColor);
  tft.drawArc(9 + x, 14 + y, 4, 6, 130, 230, willyConfig.priColor,
              willyConfig.bgColor);
  tft.drawArc(9 + x, 14 + y, 10, 12, 130, 230, willyConfig.priColor,
              willyConfig.bgColor);
}
...
void drawWebUISmall(int x, int y) {
  ...
  tft.drawCircle(8 + x, 8 + y, 7, willyConfig.priColor);

  tft.drawLine(3 + x, 4 + y, 14 + x, 4 + y, willyConfig.priColor);
  tft.drawLine(2 + x, 8 + y, 15 + x, 8 + y, willyConfig.priColor);
  tft.drawLine(3 + x, 12 + y, 14 + x, 12 + y, willyConfig.priColor);
}

void drawBLESmall(int x, int y) {
  tft.fillRect(x, 2 + y, 17, 13, willyConfig.bgColor);
  tft.drawWideLine(8 + x, 8 + y, 4 + x, 5 + y, 2, willyConfig.priColor,
                   willyConfig.bgColor);
  tft.drawWideLine(8 + x, 8 + y, 4 + x, 13 + y, 2, willyConfig.priColor,
                   willyConfig.bgColor);
  tft.drawTriangle(8 + x, 8 + y, 8 + x, 2 + y, 13 + x, 5 + y,
                   willyConfig.priColor);
  tft.drawTriangle(8 + x, 8 + y, 8 + x, 14 + y, 13 + x, 11 + y,
                   willyConfig.priColor);
}

void drawBLE_beacon(int x, int y, uint16_t color) {
  tft.fillRect(x, y, 40, 80, willyConfig.bgColor);
  tft.drawWideLine(40 + x, 53 + y, 2 + x, 26 + y, 5, color,
                   willyConfig.bgColor);
  tft.drawWideLine(40 + x, 26 + y, 2 + x, 53 + y, 5, color,
                   willyConfig.bgColor);
  tft.drawWideLine(40 + x, 53 + y, 20 + x, 68 + y, 5, color,
                   willyConfig.bgColor);
  tft.drawWideLine(40 + x, 26 + y, 20 + x, 12 + y, 5, color,
                   willyConfig.bgColor);
  tft.drawWideLine(20 + x, 12 + y, 20 + x, 68 + y, 5, color,
                   willyConfig.bgColor);
...
void drawGPS(int x, int y) {
  tft.fillRect(x, y, 80, 80, willyConfig.bgColor);
  tft.drawEllipse(40 + x, 70 + y, 15, 8, willyConfig.priColor);
  tft.drawArc(40 + x, 25 + y, 23, 7, 0, 340, willyConfig.priColor,
              willyConfig.bgColor);
  tft.fillTriangle(40 + x, 70 + y, 20 + x, 64 + y, 60 + x, 64 + y,
                   willyConfig.priColor);
}

void drawGpsSmall(int x, int y) {
  tft.fillRect(x, y, 17, 17, willyConfig.bgColor);
  tft.drawEllipse(9 + x, 14 + y, 4, 3, willyConfig.priColor);
  tft.drawArc(9 + x, 6 + y, 5, 2, 0, 340, willyConfig.priColor,
              willyConfig.bgColor);
  tft.fillTriangle(9 + x, 15 + y, 5 + x, 9 + y, 13 + x, 9 + y,
                   willyConfig.priColor);
}

void drawCreditCard(int x, int y) {
  tft.fillRect(x, y, 70, 50, willyConfig.bgColor);
  tft.fillRoundRect(x + 5, y + 5, 60, 40, 5, willyConfig.priColor);
  tft.fillRect(x + 5, y + 15, 60, 10,
               getColorVariation(willyConfig.priColor, 3, -1));
  tft.fillRect(x + 10, y + 30, 12, 10,
               getColorVariation(willyConfig.priColor, 3, 1));
  tft.drawRect(x + 10, y + 30, 12, 10,
               getColorVariation(willyConfig.priColor, 5, -1));
  tft.drawRect(x + 10 + 4, y + 30, 4, 10,
               getColorVariation(willyConfig.priColor, 5, -1));
  tft.drawRect(x + 10, y + 33, 5, 4,
               getColorVariation(willyConfig.priColor, 5, -1));
  tft.drawRect(x + 17, y + 33, 5, 4,
               getColorVariation(willyConfig.priColor, 5, -1));
  tft.fillRect(x + 30, y + 35, 30, 5,
               getColorVariation(willyConfig.priColor, 5, 1));
}

void drawMfkey32Icon(int x, int y) {
  tft.drawRect(x + 2, y + 15, 24, 40, willyConfig.priColor);
  tft.drawRect(x + 5, y + 18, 18, 12, willyConfig.priColor);
  tft.drawRect(x + 5, y + 34, 18, 18, willyConfig.priColor);
  tft.drawLine(x + 5, y + 40, x + 22, y + 40, willyConfig.priColor);
  tft.drawLine(x + 5, y + 46, x + 22, y + 46, willyConfig.priColor);
  tft.drawLine(x + 11, y + 34, x + 11, y + 51, willyConfig.priColor);
  tft.drawLine(x + 17, y + 34, x + 17, y + 51, willyConfig.priColor);
  tft.drawRect(x + 30, y + 10, 25, 35, willyConfig.priColor);
  int startX = x + 32;
  int startY = y + 12;
  int endX = x + 52;
  int endY = y + 32;
  ...
  while (startX <= endX && startY <= endY && turns < 3) {
    for (int i = startX; i <= endX; i++) {
      tft.drawPixel(i, startY, willyConfig.priColor);
    }
    startY += step;
    for (int i = startY; i <= endY; i++) {
      tft.drawPixel(endX, i, willyConfig.priColor);
    }
    endX -= step;
    for (int i = endX; i >= startX; i--) {
      tft.drawPixel(i, endY, willyConfig.priColor);
    }
    endY -= step;
    for (int i = endY; i >= startY; i--) {
      tft.drawPixel(startX, i, willyConfig.priColor);
    }
    startX += step;
    turns++;
  }
  tft.fillRect(x + 40, y + 36, 6, 6,
               getColorVariation(willyConfig.priColor, 3, 1));
}

void drawMfkey64Icon(int x, int y) {
  drawMfkey32Icon(x, y);
  tft.fillRoundRect(x + 40, y + 6, 24, 14, 4, willyConfig.bgColor);
  tft.drawRoundRect(x + 40, y + 6, 24, 14, 4,
                    getColorVariation(willyConfig.priColor, 3, -1));
  tft.drawCircle(x + 48, y + 12, 4,
                 getColorVariation(willyConfig.priColor, 3, -1));
}
...
void jpegRender(int xpos, int ypos) {
  ...
  while (JpegDec.read()) { // While there is more data in the file
    ...
    if (mcu_x + mcu_w <= (int)max_x)
      win_w = mcu_w;
    else
      win_w = min_w;

    // check if the image block size needs to be changed for the bottom edge
    if (mcu_y + mcu_h <= (int)max_y)
      win_h = mcu_h;
    else
      win_h = min_h;
    ...
    if ((mcu_x + (int)win_w) <= (int)tft.width() &&
        (mcu_y + (int)win_h) <= (int)tft.height())
...
//  derived from
//  https://github.com/bitbank2/AnimatedGIF/blob/master/examples/TFT_eSPI_memory/TFT_eSPI_memory.ino
//  and
  ...
  if (pDraw->ucHasTransparency) { // if transparency used
    ...
    while (x < iWidth) {
      ...
      if (iCount) { // any opaque pixels?
        tft.drawPixel(0, 0, 0);
        tft.pushImage(pDraw->iX + x + position->x, y + position->y, iCount, 1,
                      (uint16_t *)usTemp);
        x += iCount;
        iCount = 0;
      }
    ...
    tft.drawPixel(0, 0, 0);
    tft.pushImage(pDraw->iX + position->x, y + position->y, iWidth, 1,
                  (uint16_t *)usTemp);
  }
} /* GIFDraw() */
...
bool Gif::openGIF(FS *fs, const char *filename) {
  ...
  if (gif->open(filename, openFile, closeFile, readFile, seekFile, GIFDraw)) {
    return true;
  }
...
int Gif::playFrame(int x, int y, bool bSync) {
  if (bSync && ((millis() - lTime) >= (uint32_t)*delayMilliseconds)) {
    lTime = millis();
    gifPosition.x = x;
    gifPosition.y = y;
...
bool showGif(FS *fs, const char *filename, int x, int y, bool center,
  ...
  do {
    ...
    if (check(AnyKeyPress, resetButtonStatus))
      break;

    if (playDurationMs > 0 && (millis() - timeStart) > (uint32_t)playDurationMs)
      break;
    if (playDurationMs == 0 && result == 0)
      break;
  } while (result >= 0);
...
uint16_t getComplementaryColor(uint16_t color) {
  ...
  double saturation =
      (delta == 0) ? 0 : delta / (1 - std::abs(2 * lightness - 1));
  ...
  uint16_t compl_color = uint8_t(compR * 31) << 11 | uint8_t(compG * 63) << 5 |
                         uint8_t(compB * 31);

  // change black color
  if (compl_color == 0)
    compl_color = color - 0x1111;
  ...
  if (read16(bmpFS) == 0x4D42) {
    ...
    if ((read16(bmpFS) == 1) && (read16(bmpFS) == 24) && (read32(bmpFS) == 0)) {
      ...
      for (row = 0; row < h; row++) {
        ...
        tft.drawPixel(0, 0, 0); // shared TFT_Spi devices struggle to work, need
                                // call a line first sometimes
...
bool drawImg(FS &fs, String filename, int x, int y, bool center,
  ...
  if (ext.endsWith("jpg"))
    ...
    return drawBmp(fs, filename, x, y, center);
  else if (ext.endsWith("png"))
    return drawPNG(fs, filename, x, y, center);

#if !defined(LITE_VERSION)

  else if (ext.endsWith("gif"))
    return showGif(&fs, filename.c_str(), x, y, center, playDurationMs,
                   resetButtonStatus);
#endif
  else
...
// Optionally use heap capabilities on ESP32 to pick the best memory region for
// the decoder
#if defined(ESP32)
...
int PNGDraw(PNGDRAW *pDraw) {
  ...
  // static uint16_t dmaBuffer[MAX_IMAGE_WIDTH]; // static so buffer persists
  // after fn exit
  uint8_t r = ((uint16_t)willyConfig.bgColor & 0xF800) >> 8;
  uint8_t g = ((uint16_t)willyConfig.bgColor & 0x07E0) >> 3;
  uint8_t b = ((uint16_t)willyConfig.bgColor & 0x001F) << 3;
  ...
  if (pngBinOut) {
    pngBinOut->write((uint8_t *)usPixels, pDraw->iWidth * sizeof(uint16_t));
  }
  return 1;
}
...
static bool drawPngBin(FS &fs, const String &binPath, int x, int y,
                       bool center) {
...
bool drawPNG(FS &fs, String filename, int x, int y, bool center) {
  ...
  if (fs.exists(binPath)) {
    if (pngCacheOnly)
      return true; // cache already ready
    if (drawPngBin(fs, binPath, x, y, center))
      return true;
    fs.remove(binPath); // stale cache, fall back to decode
  }

  // Allocate decoder only while drawing, then release to keep RAM available for
  // Wi-Fi/AP usage
#if defined(ESP32)
  ...
  if (!mem) {
    ...
    willyConfig.theme.label = true;
    return false;
  }

  png = new (mem) PNG();
  int16_t rc =
      png->open(filename.c_str(), myOpen, myClose, myRead, mySeek, PNGDraw);
  if (rc == PNG_SUCCESS) {
    ...
    if (pngBinOut) {
      ...
      if (fs.exists(binPath) && rc != PNG_SUCCESS)
        fs.remove(binPath);
    }
    if (rc != PNG_SUCCESS && fs.exists(binPath)) {
      fs.remove(binPath);
    }
  ...
  // Destroy placement-new object and free memory so RAM is available after
  // rendering
  png->~PNG();
#if defined(ESP32)
  if (usedHeapCaps)
