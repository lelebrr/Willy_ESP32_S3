 ...
 */
void MifareKeysManager::ensureLoaded(std::set<String> &keys, bool allowSD) {
  if (!keys.empty())
    return; // Already loaded

  // Try loading from file (SD priority)
  if ((allowSD && setupSdCard() && SD.exists(KEYS_PATH)) ||
      LittleFS.exists(KEYS_PATH)) {
    loadFromFile(keys, allowSD);
  } else {
...
void MifareKeysManager::loadFromFile(std::set<String> &keys, bool allowSD) {
  FS *sourceFS = nullptr;
  bool fromSD = false;
  ...
  log_i("Loaded %d keys%s", loaded,
        (skipped > 0 ? " (" + String(skipped) + " skipped)" : "").c_str());
}

bool MifareKeysManager::copyFileToFS(FS *sourceFS, FS *destFS,
                                     const char *destFsName) {
...
void MifareKeysManager::saveToFile(const std::set<String> &keys) {
  ...
  if (setupSdCard()) {
    sdSuccess = writeToFS(&SD, "SD", keys);
  }
...
void MifareKeysManager::appendToFile(const String &key) {
  ...
  if (setupSdCard()) {
    sdSuccess = appendToFS(&SD, "SD", key);
  }
...
bool MifareKeysManager::writeToFS(FS *fs, const char *fsName,
  ...
  file.println(
      "// [WARNING] Use for academic study only. Unauthorized use is illegal.");
...
bool MifareKeysManager::appendToFS(FS *fs, const char *fsName,
                                   const String &key) {
  if (!fs->exists(KEYS_DIR)) {
    fs->mkdir(KEYS_DIR);
  }
