...
void updateClockTimezone() {
    ...
    timeClient->setTimeOffset(willyConfig.tmz * 3600);

    localTime = timeClient->getEpochTime() + (willyConfig.dst ? 3600 : 0);
...
void showDeviceInfo() {
    ...
    area.addLine("Willy Version: " + String(WILLY_VERSION));
    ...
    area.addLine("Brightness: " + String(willyConfig.bright) + "%");
    area.addLine("");
#endif

    area.addLine("[GPIO]");
    area.addLine("GROVE_SDA: " + String(willyConfigPins.i2c_bus.sda));
    area.addLine("GROVE_SCL: " + String(willyConfigPins.i2c_bus.scl));
    area.addLine("SERIAL TX: " + String(willyConfigPins.uart_bus.tx));
    area.addLine("SERIAL RX: " + String(willyConfigPins.uart_bus.rx));
...
String getOptionsJSON() {
    ...
    for (auto opt : options) {
        ...
        i++;
        if ((size_t)i < options.size()) response += ",";
    }
...
void i2c_bulk_write(TwoWire *wire, uint8_t addr, const uint8_t *bulk_data) {
    ...
    while (true) {
        ...
        wire->write(reg);
        for (uint8_t i = 0; i < (uint8_t)(datalen - 1); i++) { wire->write(*p++); }
