#ifndef WILLY_WILLY_USBSERIAL_H
#define WILLY_WILLY_USBSERIAL_H

#include "SerialDevice.h"
#include <Arduino.h>
#include <Stream.h>

class WillyUSBSerial : public SerialDevice {
public:
  ...
  size_t println(const int n, int format) override {
    return out->println(n, format);
  }
  String readStringUntil(char terminator) override {
    return out->readStringUntil(terminator);
  }
  ...
  WillyUSBSerial(Stream *in) { out = in; }
  ~WillyUSBSerial() override = default;

private:
  Stream *out;
};

#endif // WILLY_WILLY_USBSERIAL_H
