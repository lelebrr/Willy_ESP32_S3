...
// Helper function to format time_t as local time string
    ...
    if (esp_get_free_heap_size() >
      ...
      if (isDir) {
        ...
        if (file) {
          ...
          serialDevice->println(formatLocalTime(modTime));
          file.close();
        }
      }
    } else
...
uint32_t writeCallback(cmd *c) {
  ...
  if (fileSize < SAFE_STACK_BUFFER_SIZE)
    fileSize = SAFE_STACK_BUFFER_SIZE;
...
uint32_t renameCallback(cmd *c) {
  ...
  if (filepath.length() == 0 || newName.length() == 0)
    return false;
...
uint32_t statCallback(cmd *c) {
  ...
  serialDevice->print("\t\t");
  if (file.isDirectory())
    serialDevice->print("directory");
  else
    serialDevice->print("regular file");
