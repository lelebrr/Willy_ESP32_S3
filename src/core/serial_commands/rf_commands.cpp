...
uint32_t rfTxCallback(cmd *c) {
  ...
  // Calculate bits from key length (each hex digit = 4 bits)
  unsigned int bits = strKey.length() * 4;
  if (bits == 0)
    bits = 24; // fallback to 24 bits

  // check valid frequency and init the rf module
  if (!initRfModule("tx", float(frequency / 1000000.0)))
    return false;
...
uint32_t rfScanCallback(cmd *c) {
  ...
  if (startFreq < 300000000 || stopFreq > 928000000 || startFreq >= stopFreq) {
    serialDevice->println(
        "Invalid range. Must be 300-928 MHz and start < stop.");
    return false;
  }
...
uint32_t rfSendCallback(cmd *c) {
  // tasmota json command
  // https://tasmota.github.io/docs/Tasmota-IR/#sending-ir-commands e.g. RfSend
  // {\"Data\":\"0x447503\",\"Bits\":24,\"Protocol\":1,\"Pulse\":174,\"Repeat\":10}
  // // on e.g. RfSend
  // {\"Data\":\"0x44750C\",\"Bits\":24,\"Protocol\":1,\"Pulse\":174,\"Repeat\":10}
  // // off
  ...
  int pulse =
      0; // 0 leave the library use the default value depending on protocol
  int repeat = 10;
