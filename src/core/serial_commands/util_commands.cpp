...
uint32_t dateCallback(cmd *c) {
  ...
  _rtc.GetDate(&_date);
  char stimeStr[100] = {0};
  snprintf(stimeStr, sizeof(stimeStr), "%02d %02d %04d %02d:%02d:%02d",
           _date.Month, _date.Date, _date.Year, _time.Hours, _time.Minutes,
           _time.Seconds);
...
uint32_t infoCallback(cmd *c) {
  ...
  // ESP.getChipRevision()); serialDevice->printf("Detected flash size: %d\n",
  // ESP.getFlashChipSize()); serialDevice->printf("This chip has %d cores\n",
  // ESP.getChipCores()); serialDevice->printf("CPU Freq is %d\n",
  // ESP.getCpuFreqMHz()); Features: WiFi, BLE, Embedded Flash 8MB (GD) Crystal
  // is 40MHz MAC: 24:58:7c:5b:24:5c
...
uint32_t helpCallback(cmd *c) {
  ...
  serialDevice->println("  wifi on  (Connects to a known Wifi network. if "
                        "there's no known network, starts in AP Mode)");
  serialDevice->println(
  ...
  serialDevice->println("\nIR Commands:");
  serialDevice->println("  ir rx <timeout>      - Read an IR signal and print "
                        "the dump on serialDevice->");
  serialDevice->println("  ir rx raw <timeout>  - Read an IR signal in RAW "
                        "mode and print the dump on serialDevice->");
  serialDevice->println("  ir tx <protocol> <address> <decoded_value>  - Send "
                        "a custom decoded IR signal.");
  serialDevice->println("  ir tx_from_file <ir file path> [hide default UI "
                        "true/false] - Send an IR signal saved in "
                        "storage. Optionally hide the default UI.");

  serialDevice->println("\nRF Commands:");
  serialDevice->println("  subghz rx <timeout>       - Read an RF signal and "
                        "print the dump on serialDevice-> (alias: rf rx)");
  serialDevice->println("  subghz rx raw <timeout>   - Read an RF signal in "
                        "RAW mode and print the dump on serialDevice-> "
                        "(alias: "
                        "rf rx raw)");
  serialDevice->println("  subghz tx <decoded_value> <frequency> <te> <count>  "
                        "- Send a custom decoded RF signal. (alias: rf "
                        "tx)");
  serialDevice->println("  subghz tx_from_file <sub file path> [hide default "
                        "UI true/false] - Send an RF signal "
                        "saved in storage. Optionally hide the default UI.");

  serialDevice->println("\nAudio Commands:");
  serialDevice->println(
      "  music_player <audio file path>  - Play an audio file.");
  serialDevice->println(
      "  tone <frequency> <duration>  - Play a single squarewave audio tone.");
  ...
  serialDevice->println("\nGPIO Commands:");
  serialDevice->println("  gpio mode <pin number> <0/1>  - Set GPIO pins mode "
                        "(0=input, 1=output).");
  serialDevice->println("  gpio set <pin number> <0/1>   - Direct GPIO pins "
                        "control (0=off, 1=on).");

  serialDevice->println("\nI2C and Storage:");
  serialDevice->println(
      "  i2c scan                - Scan for modules connected to the I2C bus.");
  serialDevice->println(
      "  storage <list/remove/mkdir/rename/read/write/copy/md5/crc32> <file "
      "path>  - Common file "
      "management commands.");
  ...
  serialDevice->println("\nSettings:");
  serialDevice->println(
      "  settings                - View all the current settings.");
  serialDevice->println(
      "  settings <name>         - View a single setting value.");
  serialDevice->println(
      "  settings <name> <new value>  - Alter a single setting value.");
  serialDevice->println(
      "  factory_reset           - Reset to default configuration.");
...
uint32_t navCallback(cmd *c) {
  ...
  if (_dur.length() > 0) {
    ...
    if (dur < 0)
      dur = 1; // heavy fingers on the remote interface
  }
  ...
  if (nav == "next") {
    ...
    serialDevice->println("Unknown command, use: \n\"nav Next\" or \n\"nav "
                          "Prev\" or \n\"nav Esc\" or \n\"nav Select\" or "
                          "\n\"nav Up\" or \n\"nav Down\" or \n\"nav "
                          "NextPage\" or \n\"nav PrevPage\"");
    return false;
  }
  // wakeUpScreen(); // Do not wakeup screen if it is dimmed and using Remote
  // control
...
uint32_t optionsCallback(cmd *c) {
  ...
  if (opt >= 0 && (size_t)opt < options.size()) {
    // wakeUpScreen(); // Do not wakeup screen if it is dimmed and using Remote
    // control
    forceMenuOption = opt;
    serialDevice->printf("Selected option %d: %s\n", forceMenuOption,
                         options[forceMenuOption].label.c_str());
    vTaskDelay(30 / portTICK_PERIOD_MS);
    optionsList();
  } else if (options.size() > 0) {
    optionsList();
  } else
    serialDevice->println("No options Available");
  return true;
}
...
uint32_t displayCallback(cmd *c) {
  ...
  if (opt == "start") {
    ...
    if (tft.getLogging())
      serialDevice->println("Display: Logging tft is ACTIVATED");
    else
      serialDevice->println("Display: Logging tft is DEACTIVATED");
  } else if (opt == "dump") {
    ...
    serialDevice->println(TFT_WIDTH + String("x") + TFT_HEIGHT + String("x") +
                          ROTATION);
  } else {
    serialDevice->println("Display command accept:\n"
...
uint32_t loaderCallback(cmd *c) {
  ...
  if (arg == "list") {
    for (int i = 0; i < _totalItems; i++) {
      serialDevice->println(_menuItems[i]->getName());
    }
    ...
    if (!appname.isEmpty()) {
      // Set the current loader app before opening
      currentLoaderApp = appname;
      appRequiresClose = true;
...
// additional shortcuts
      ...
      currentLoaderApp = "";
      appRequiresClose = false;
      return false;
    }

  } else if (arg == "close") {
    // Close the currently running application
    if (!appRequiresClose || currentLoaderApp.isEmpty()) {
      serialDevice->println("No application is currently running.");
      return false;
    }

    serialDevice->println("Closing application: " + currentLoaderApp);

    // Set returnToMenu to break any active loops
    returnToMenu = true;

    // Clean up based on the current app
    if (currentLoaderApp.equalsIgnoreCase("webui")) {
      // Stop WebUI if running
      if (isWebUIActive) {
        // WebUI cleanup would go here (stop server, etc.)
        isWebUIActive = false;
      }
    } else if (currentLoaderApp.equalsIgnoreCase("badusb")) {
      // BadUSB cleanup
      // Note: ducky_setup doesn't have a direct cleanup, but we can reset the
      // HID The hid_usb pointer is managed elsewhere
    } else if (currentLoaderApp.equalsIgnoreCase("littlefs")) {
      // LittleFS loopSD returns automatically, just reset flag
    }
    ...
    // Display loader state information
    serialDevice->println("Loader State:");
    if (appRequiresClose && !currentLoaderApp.isEmpty()) {
      serialDevice->println("  Current app: " + currentLoaderApp);
    } else {
      serialDevice->println("  No application running");
    }
    serialDevice->println("  WebUI active: " +
                          String(isWebUIActive ? "yes" : "no"));
    serialDevice->println("  WiFi connected: " +
                          String(wifiConnected ? "yes" : "no"));
    if (wifiConnected) {
      serialDevice->println("  IP: " + wifiIP);
    }
    return true;
