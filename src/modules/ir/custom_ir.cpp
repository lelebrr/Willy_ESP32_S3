...
void selectRecentIrMenu() {
  ...
  for (auto recent_ircode : recent_ircodes) {
    if (recent_ircode->filepath == "")
      continue; // not inited
    // else
    options.push_back(
        {recent_ircode->filepath.c_str(),
         [recent_ircode, &selected_code]() { selected_code = recent_ircode; }});
  }
...
bool txIrFile(FS *fs, String filepath, bool hideDefaultUI) {
  ...
  // digitalWrite(willyConfigPins.irTx, LED_ON);
  ...
  resetCodesArray();
  digitalWrite(willyConfigPins.irTx, LED_OFF);
  return true;
}
...
void sendIRCommand(IRCode *code, bool hideDefaultUI) {
  ...
  // Others protocols of IRRemoteESP8266, not related to Flipper Zero IR File
  // Format
...
void sendNECCommand(String address, String command, bool hideDefaultUI) {
  IRsend irsend(
      willyConfigPins.irTx); // Set the GPIO to be used to sending the message.
  irsend.begin();
  if (!hideDefaultUI) {
  ...
  if (willyConfigPins.irTxRepeats > 0) {
    for (uint8_t i = 1; i <= willyConfigPins.irTxRepeats; i++) {
      irsend.sendNEC(data, 32);
    }
  }

  Serial.println("Sent NEC Command" +
                 (willyConfigPins.irTxRepeats > 0
                      ? " (1 initial + " + String(willyConfigPins.irTxRepeats) +
                            " repeats)"
                      : ""));
...
void sendNECextCommand(String address, String command, bool hideDefaultUI) {
  ...
  if (first_zero_byte_pos != -1)
    address = address.substring(0, first_zero_byte_pos);
  first_zero_byte_pos = command.indexOf("00", 2);
  if (first_zero_byte_pos != -1)
    command = command.substring(0, first_zero_byte_pos);
...
void sendRC5Command(String address, String command, bool hideDefaultUI) {
  IRsend irsend(willyConfigPins.irTx,
                true); // Set the GPIO to be used to sending the message.
  irsend.begin();
  if (!hideDefaultUI) {
    displayTextLine("Sending..");
  }
...
void sendRC6Command(String address, String command, bool hideDefaultUI) {
  ...
  Serial.println("Sent RC6 Command" +
                 (willyConfigPins.irTxRepeats > 0
                      ? " (1 initial + " + String(willyConfigPins.irTxRepeats) +
                            " repeats)"
                      : ""));
...
bool sendDecodedCommand(String protocol, String value, uint8_t bits,
  ...
  if (hasACState(type)) {
    ...
    success = irsend.send(
        type, value_int,
        bits); // bool send(const decode_type_t type, const uint64_t data,
...
void sendRawCommand(uint16_t frequency, String rawData, bool hideDefaultUI) {
  ...
  uint16_t *dataBuffer =
      (uint16_t *)malloc((dataBufferSize) * sizeof(uint16_t));
...
bool chooseCmdIrFile(FS *fs, String filepath) {
  ...
  while (databaseFile.available() && total_codes < 100) {
    ...
    if (line.startsWith("name:")) {
      ...
      codes[total_codes]->filepath =
          txt + " " + filepath.substring(1 + filepath.lastIndexOf("/"));
    }

    if (line.startsWith("type:"))
      codes[total_codes]->type = txt;
    if (line.startsWith("protocol:"))
      codes[total_codes]->protocol = txt;
    if (line.startsWith("address:"))
      codes[total_codes]->address = txt;
    if (line.startsWith("frequency:"))
      codes[total_codes]->frequency = txt.toInt();
    if (line.startsWith("bits:"))
      codes[total_codes]->bits = txt.toInt();
    if (line.startsWith("command:"))
      codes[total_codes]->command = txt;
    if (line.startsWith("data:") || line.startsWith("value:") ||
        line.startsWith("state:")) {
      codes[total_codes]->data = txt;
    }

    if (line.startsWith("#") && total_codes < codes.size() &&
        codes[total_codes]->name != "") {
      total_codes++;
      codes.push_back(new IRCode());
    }
  }
...
bool txIrData(const char *data, bool hideDefaultUI) {
  // SPAM all codes from data string
  ...
  while (end != -1 || start < dataStr.length()) {
    if (end == -1) {
      // Last line
      line = dataStr.substring(start);
      if (line.endsWith("\r"))
    ...
    if (line.startsWith("type:")) {
      // Send previous code if exists
      if (protocol != "" || rawData != "") {
        if (protocol == "RAW") {
          sendRawCommand(frequency, rawData, hideDefaultUI);
        } else if (protocol != "") {
          sendDecodedCommand(protocol, value, bits, hideDefaultUI);
        }
        codes_sent++;
        delay(100);
      }
      ...
      protocol = line.substring(line.indexOf(":") + 1);
      protocol.trim();
    } else if (line.startsWith("address:")) {
      ...
      frequency = line.substring(line.indexOf(":") + 1).toInt();
    } else if (line.startsWith("bits:")) {
      bits = line.substring(line.indexOf(":") + 1).toInt();
    } else if (line.startsWith("RAW_Data:")) {
      rawData = line.substring(line.indexOf(":") + 1);
      rawData.trim();
