...
String uint32ToString(uint32_t value) {
  ...
  snprintf(buffer, sizeof(buffer), "%02lX %02lX %02lX %02lX", value & 0xFF,
...
String uint32ToStringInverted(uint32_t value) {
  char buffer[12] = {0}; // 8 hex digits + 3 spaces + 1 null terminator
  snprintf(buffer, sizeof(buffer), "%02lX %02lX %02lX %02lX",
...
void IrRead::setup() {
  ...
  int count = 0;
  for (auto pin : pins) {
    if (pin.second == willyConfigPins.irRx)
      count++;
  }
  if (count == 0)
    gsetIrRxPin(true); // Open dialog to choose irRx pin

  setup_ir_pin(willyConfigPins.irRx, INPUT_PULLUP);
  if (headless)
    return;
  // else
...
void IrRead::loop() {
  ...
  while (1) {
    ...
    if (check(NextPress))
      save_signal();
    if (quickloop && button_pos == (int)quickButtons.size())
      save_device();
    if (check(SelPress))
      save_device();
    if (check(PrevPress))
...
void IrRead::begin() {
  ...
  display_banner();
  if (quickloop) {
    padprintln("Aguardando sinal do botao: " +
               String(quickButtons[button_pos]));
  } else {
...
void IrRead::display_btn_options() {
  ...
  if (signals_read > 0) {
    padprintln("Pressione [OK]   p/ salvar disp");
  }
...
void IrRead::read_signal() {
...
#endif
  if (_read_signal || !irrecv.decode(&results))
    return;
  ...
  tft.println(raw_signal.substring(0, 45) +
              (raw_signal.length() > 45
                   ? "..."
...
void IrRead::append_to_file_str(String btn_name) {
  ...
  if (raw) {
    ...
    if (hasACState(results.decode_type))
      ...
      strDeviceContent +=
          "value: " + uint32ToStringInverted(results.value) + "\n";
    ...
    uint32ToString(results.value>> 32); value.replace(" ", ""); uint64_t
    value_int = strtoull(value.c_str(), nullptr, 16); Serial.println(value_int);
    */
  }
  strDeviceContent += "#\n";
}
...
void IrRead::save_device() {
  ...
  if (fs && write_file(filename, fs)) {
    displaySuccess("Arq salvo no " +
                       String((fs == &SD) ? "SD Card" : "LittleFS") + ".",
                   true);
    signals_read = 0;
    strDeviceContent = "";
  } else
    displayError(fs ? "Erro ao gravar arq." : "Sem armaz. disponivel.", true);
...
bool IrRead::write_file(String filename, FS *fs) {
  ...
  while ((*fs).exists("/WillyIR/" + filename + ".ir")) {
    ...
    case 1:
      filename += "_";
      while ((*fs).exists("/WillyIR/" + filename + String(i) + ".ir"))
        i++;
      filename += String(i);
      break;
    case 2:
      (*fs).remove("/WillyIR/" + filename + ".ir");
      break;
    case 3:
  ...
  */
  ...
  file.println("Filetype: Willy IR File");
