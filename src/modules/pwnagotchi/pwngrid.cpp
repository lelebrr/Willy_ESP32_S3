...
* @7h30th3r0n3 : https://github.com/7h30th3r0n3/Evil-M5Core2 and
https://github.com/7h30th3r0n3/PwnGridSpam
...
20/09 - Changed from DynamicJsonDocument json[2048] to JsonDocument json, to
avoid stack smashing errors (firgers crossed)
*/
...
void delete_peer_gone() {     // Delete peers wigh pwngrid_peers.gone = true
  std::vector<int> peer_gone; // Create a vector of integers to save the index
                              // value of the element to be deleted
  int index = 0;
  for (const auto &peer_list : pwngrid_peers) {
    if (peer_list.gone)
      peer_gone.push_back(index); // Saves the index value into the vector
    index++;
  }
  std::reverse(peer_gone.begin(),
               peer_gone.end()); // Reverse the vector to iterate from the end
                                 // to the beginning
  for (auto ind : peer_gone) {
    pwngrid_peers.erase(pwngrid_peers.begin() +
                        ind); // Delete the peer from the list
    // Update counter;
    pwngrid_friends_tot = pwngrid_peers.size();
  }
  peer_gone.clear();
...
esp_err_t pwngridAdvertise(uint8_t channel, String face) {
  ...
  uint8_t *pwngrid_beacon_frame =
      (uint8_t *)heap_caps_malloc(frame_len, MALLOC_CAP_8BIT);
  ...
  if (_chan != channel) {
    ...
    if (err != ESP_OK) {
      ESP_LOGE("pwngrid", "Failed to set channel %d: %s", channel,
               esp_err_to_name(err));
    }
  ...
  for (int i = 0; i < 3; i++) {
    result =
        esp_wifi_80211_tx(WIFI_IF_AP, pwngrid_beacon_frame, frame_len, false);
    if (result != ESP_OK) {
      ESP_LOGE("pwngrid", "Failed sending advertising: %s",
...
void pwnSnifferCallback(void *buf, wifi_promiscuous_pkt_type_t type) {
  ...
  if (frameType == 0x00 && frameSubType == 0x08) {
    const uint8_t *addr1 =
        snifferPacket->payload + 4; // Adresse du destinataire (Adresse 1)
    const uint8_t *addr2 =
        snifferPacket->payload + 10; // Adresse de l'expéditeur (Adresse 2)
    const uint8_t *bssid =
        snifferPacket->payload + 16; // Adresse BSSID (Adresse 3)
  ...
  if (type == WIFI_PKT_MGMT) {
    ...
    // *)snifferPacket->payload; const WifiMgmtHdr *hdr = &ipkt->hdr;
