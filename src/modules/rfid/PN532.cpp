...
PN532::PN532(CONNECTION_TYPE connection_type) {
  ...
  if (connection_type == CONNECTION_TYPE::I2C)
    ...
    nfc.setInterface(GPIO_NUM_26, GPIO_NUM_25);
#endif
  else
    nfc.setInterface(SPI_SCK_PIN, SPI_MISO_PIN, SPI_MOSI_PIN, SPI_SS_PIN);
}
...
int PN532::read(int cardBaudRate) {
  ...
  if (cardBaudRate == PN532_MIFARE_ISO14443A) {
    if (!nfc.startPassiveTargetIDDetection(cardBaudRate))
    ...
    if (!nfc.felica_Polling(sys_code, req_code, idm, pmm, &sys_code_res)) {
      return TAG_NOT_PRESENT;
    }
  ...
  willyLogger.logNFC(printableUID.uid.c_str(), printableUID.picc_type.c_str(),
                     true);
...
int PN532::clone() {
  ...
  if (nfc.mifareclassic_WriteBlock0(data)) {
    ...
    uint8_t num = 0;
    while ((!nfc.startPassiveTargetIDDetection() ||
    ...
    if (authenticate_mifare_classic(0) == SUCCESS &&
        nfc.mifareclassic_WriteDataBlock(0, data)) {
      return SUCCESS;
    }
  }
  return FAILURE;
...
int PN532::load() {
  ...
  while (file.available()) {
    ...
    if (line.startsWith("Device type:"))
      printableUID.picc_type = strData;
    if (line.startsWith("UID:"))
      printableUID.uid = strData;
    if (line.startsWith("SAK:"))
      printableUID.sak = strData;
    if (line.startsWith("ATQA:"))
      printableUID.atqa = strData;
...
void PN532::set_uid() {
  ...
  for (byte i = 0; i < 2; i++)
    uid.atqaByte[i] = nfc.targetUid.atqaByte[i];
...
void PN532::format_data_felica(uint8_t idm[8], uint8_t pmm[8],
                               uint16_t sys_code) {
...
int PN532::read_mifare_classic_data_sector(byte sector) {
  ...
  for (int8_t blockOffset = 0; blockOffset < no_of_blocks; blockOffset++) {
    ...
    if (!nfc.mifareclassic_ReadDataBlock(blockAddr, buffer))
      return FAILURE;
...
int PN532::authenticate_mifare_classic(byte block) {
  ...
  for (auto key : keys) {
    successA = nfc.mifareclassic_AuthenticateBlock(uid.uidByte, uid.size, block,
                                                   0, key);
    if (successA)
      break;
  ...
  if (!successA) {
    ...
    for (const auto &mifKey : willyConfig.mifareKeys) {
      ...
      successA = nfc.mifareclassic_AuthenticateBlock(uid.uidByte, uid.size,
                                                     block, 0, keyA);
      if (successA)
        break;
  ...
  for (auto key : keys) {
    successB = nfc.mifareclassic_AuthenticateBlock(uid.uidByte, uid.size, block,
                                                   1, key);
    if (successB)
      break;
  ...
  if (!successB) {
    ...
    for (const auto &mifKey : willyConfig.mifareKeys) {
      ...
      successB = nfc.mifareclassic_AuthenticateBlock(uid.uidByte, uid.size,
                                                     block, 1, keyB);
      if (successB)
        break;
...
int PN532::read_felica_data() {
  ...
  for (uint16_t i = 0x8000; i < 0x8000 + totalPages; i++) {
    ...
    uint16_t default_service_code[1] = {
        0x000B}; // Default service code for reading. Should works for every
                 // card
    int res = nfc.felica_ReadWithoutEncryption(1, default_service_code, 1,
    ...
    if (res) { // If PN532 can't read the FeliCa tag, don't write the block to
               // file
    ...
    if (printableUID.picc_type != "FeliCa") {
      ...
      case PICC_TYPE_MIFARE_4K:
        if (pageIndex == 0 || (pageIndex + 1) % 4 == 0)
          continue;
        blockWriteSuccess =
            write_mifare_classic_data_block(pageIndex, strBytes);
        break;
      ...
      case PICC_TYPE_MIFARE_UL:
        if (pageIndex < 4 || pageIndex >= dataPages - 5)
          continue;
        blockWriteSuccess =
            write_mifare_ultralight_data_block(pageIndex, strBytes);
        break;
...
bool PN532::write_mifare_classic_data_block(int block, String data) {
  ...
  if (authenticate_mifare_classic(block) != SUCCESS)
    return false;
...
int PN532::write_felica_data_block(int block, String data) {
  ...
  for (size_t i = 0; i < data.length(); i += 2) {
    if (i / 2 < 16) {
      block_data[0][i / 2] =
          strtoul(data.substring(i, i + 2).c_str(), NULL, 16);
    }
  }

  uint16_t block_list[1] = {
      (uint16_t)(block + 0x8000)}; // Write the block i. Block in FeliCa start
                                   // from 0x8000

  uint16_t default_service_code[1] = {
      0x0009}; // Default service code for writing. Should works for every card

  return nfc.felica_WriteWithoutEncryption(1, default_service_code, block,
                                           block_list, block_data);
}
...
int PN532::erase_data_blocks() {
  ...
  case PICC_TYPE_MIFARE_4K:
    for (byte i = 1; i < 64; i++) {
      if ((i + 1) % 4 == 0)
        continue;
      blockWriteSuccess = write_mifare_classic_data_block(
...
int PN532::write_ndef_blocks() {
  ...
  byte payload_size =
      ndef_size % 4 == 0 ? ndef_size : ndef_size + (4 - (ndef_size % 4));
