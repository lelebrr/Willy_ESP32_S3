...
RFID2::RFID2(bool use_i2c) : _use_i2c(use_i2c) {
  if (use_i2c)
    _driver =
        new MFRC522DriverI2C{RFID2_I2C_ADDRESS, willyConfigPins.i2c_bus.sda,
                             willyConfigPins.i2c_bus.scl};
  else
    _driver =
        new MFRC522DriverSPI{ss_pin, SPI_SCK_PIN, SPI_MISO_PIN, SPI_MOSI_PIN};
...
int RFID2::read_data_blocks() {
  ...
  case MFRC522::PICC_Type::PICC_TYPE_MIFARE_UL:
    ...
    dataPages =
        (readStatus == SUCCESS && dataPages > 0) ? dataPages - 1 : dataPages;
    if (totalPages == 0)
...
int RFID2::authenticate_mifare_classic(byte block) {
  ...
  for (auto key : keys) {
    ...
    statusA =
        mfrc522.PCD_Authenticate(MFRC522::PICC_Command::PICC_CMD_MF_AUTH_KEY_A,
                                 block, &keyA, &mfrc522.uid);
  ...
  if (statusA != MFRC522::StatusCode::STATUS_OK) {
    for (const auto &mifKey : willyConfig.mifareKeys) {
      for (size_t i = 0; i < mifKey.length(); i += 2) {
        keyA.keyByte[i / 2] =
            strtoul(mifKey.substring(i, i + 2).c_str(), NULL, 16);
      }

      statusA = mfrc522.PCD_Authenticate(
          MFRC522::PICC_Command::PICC_CMD_MF_AUTH_KEY_A, block, &keyA,
          &mfrc522.uid);
      if (statusA == MFRC522::StatusCode::STATUS_OK)
        break;
  ...
  for (auto key : keys) {
    ...
    statusB =
        mfrc522.PCD_Authenticate(MFRC522::PICC_Command::PICC_CMD_MF_AUTH_KEY_B,
                                 block, &keyB, &mfrc522.uid);
  ...
  if (statusB != MFRC522::StatusCode::STATUS_OK) {
    for (const auto &mifKey : willyConfig.mifareKeys) {
      ...
      statusB = mfrc522.PCD_Authenticate(
          MFRC522::PICC_Command::PICC_CMD_MF_AUTH_KEY_A, block, &keyB,
          &mfrc522.uid);
      if (statusB == MFRC522::StatusCode::STATUS_OK)
        break;
  ...
  return (statusA == MFRC522::StatusCode::STATUS_OK &&
          statusB == MFRC522::StatusCode::STATUS_OK)
             ? SUCCESS
             : TAG_AUTH_ERROR;
}
...
int RFID2::read_mifare_ultralight_data_blocks() {
  ...
  for (byte page = 0; page <= 252; page += 4) {
    ...
    if (status != MFRC522::StatusCode::STATUS_OK) {
      return status == MFRC522::StatusCode::STATUS_MIFARE_NACK ? SUCCESS
                                                               : FAILURE;
    }
...
int RFID2::write_ndef_blocks() {
  ...
  for (int i = 0; i < payload_size; i += 4) {
    int block = 4 + (i / 4);
    byte status =
        mfrc522.MIFARE_Ultralight_Write((byte)block, ndef_payload + i, 4);
    if (status != MFRC522::StatusCode::STATUS_OK)
      return FAILURE;
  }
