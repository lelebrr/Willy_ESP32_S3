 ...
 * @author Senape3000
 * @info
 * https://github.com/Senape3000/firmware/blob/main/docs_custom/SRIX/SRIX_Tool_README.md
 * @date 2026-01-01
 */
#include "srix_tool.h"
...
static constexpr uint32_t SRIX_EEPROM_WRITE_DELAY_MS =
    15; // Delay for write operation
...
void SRIXTool::setup() {
  ...
  int sda_pin = willyConfigPins.i2c_bus.sda;
  int scl_pin = willyConfigPins.i2c_bus.scl;
...
void SRIXTool::select_state() {
  ...
  if (_dump_valid_from_read) {
    ...
    options.emplace_back(" -Clonar tag",
                         [this]() { set_state(WRITE_TAG_MODE); });
  }
  ...
  if (_dump_valid_from_load) {
    options.emplace_back(" -Gravar na tag",
                         [this]() { set_state(WRITE_TAG_MODE); });
  }
...
void SRIXTool::display_banner() {
  ...
  switch (current_state) {
  case READ_TAG_MODE:
    printSubtitle("MODO LER TAG");
    break;
  case WRITE_TAG_MODE:
    printSubtitle("MODO GRAVAR TAG");
    break;
  case READ_UID_MODE:
    printSubtitle("MODO LER UID");
    break;
  case PN_INFO_MODE:
    printSubtitle("PN532 INFO");
    break;
  case IDLE_MODE:
    printSubtitle("MENU PRINCIPAL");
    break;
  case SAVE_MODE:
...
void SRIXTool::save_file() {
  ...
  // Write header
  file.println("Filetype: Willy SRIX Dump");
...
void SRIXTool::load_file() {
  ...
  for (const String &fname : fileList) {
    options.emplace_back(fname, [this, fname, fs]() {
      load_file_data(fs, "/WillyRFID/SRIX/" + fname);
    });
  }
...
bool SRIXTool::waitForTagHeadless(uint32_t timeout_ms) {
  ...
  (void)0; // rfResetDone removed (was unused)
...
String SRIXTool::save_file_headless(String filename) {
  ...
  if ((*fs).exists(filepath)) {
    ...
    while ((*fs).exists(base + "_" + String(i) + ".srix"))
      i++;
...
int SRIXTool::write_single_block_headless(uint8_t block_num,
