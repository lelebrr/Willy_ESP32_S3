...
typedef enum emv_vendor { EMV_VISA, EMV_MASTERCARD, EMV_UNKNOWN } EMV_Vendor;
...
class EMVReader {
private:
  // EMV methods created with the help of
  ...
  // VISA save the card details in tag 57 (Track 2 Equivalent Data) so we need a
  ...
  void save_emv(const char *aid, const char *pan, const char *validfrom,
                const char *validto);
