...
void EMVReader::parse_validfrom(std::vector<uint8_t> *afl_content,
                                EMVCard *card) {
  bool found = false;
  for (size_t i = 0; i < afl_content->size() && !found; i++) {
    if (afl_content->at(i) == 0x5F && afl_content->at(i + 1) == 0x25) {
      ...
      // The format in card is YEAR/MONTH but I want MONTH/YEAR since is the
      // standard format
...
void EMVReader::parse_validto(std::vector<uint8_t> *afl_content,
                              EMVCard *card) {
  bool found = false;
  for (size_t i = 0; i < afl_content->size() && !found; i++) {
...
std::vector<uint8_t> EMVReader::emv_ask_for_aid() {
  ...
  while (!_cancelled) {
    ...
    if (nfc->readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &len, 100)) {
      /* Select Application */
      uint8_t ask_for_aid_apdu[] = {0x00, 0xA4, 0x04, 0x00, 0x0e, 0x32, 0x50,
                                    0x41, 0x59, 0x2e, 0x53, 0x59, 0x53, 0x2e,
                                    0x44, 0x44, 0x46, 0x30, 0x31, 0x00};
      if (nfc->EMVinDataExchange(ask_for_aid_apdu, sizeof(ask_for_aid_apdu),
                                 response, &response_len)) {
        std::vector<uint8_t> response_vector(&response[0],
                                             &response[response_len]);
        BerTlv Tlv;
...
std::vector<uint8_t> EMVReader::emv_ask_for_pdol(std::vector<uint8_t> *aid) {
  ...
  /* ------------------- AID -----------------*/
  uint8_t ask_for_pdol[] = {0x00, 0xa4, 0x04, 0x00, 0x07, 0x00, 0x00,
                            0x00, 0x00, 0x00, 0x00, 0x90, 0x00};
  memcpy(ask_for_pdol + 5, aid->data(), 7);

  if (nfc->EMVinDataExchange(ask_for_pdol, sizeof(ask_for_pdol), response,
                             &response_len)) {
    std::vector<uint8_t> response_vector(&response[0], &response[response_len]);
...
void EMVReader::emv_read_visa(std::vector<uint8_t> *pdol_data, EMVCard *card) {
  ...
  if (nfc->EMVinDataExchange(payload, sizeof(payload), response,
                             &response_len)) {
    std::vector<uint8_t> response_vector(&response[0], &response[response_len]);
    ...
    if (Tlv.GetValue("57", &container) != OK) {
      ...
      memcpy(card->pan, container.data(),
             8 * sizeof(uint8_t)); // Copy data from TLV to struct
      card->pan_len = 8;

      // Index 8 is separator 'D' and first digit of ValidTo month
      // Index 9 is second digit of ValidTo month and first digit of ValidTo
      // year Index 10 is second digit of ValidTo year and first digit of
      // Service Code
      card->validto = (uint8_t *)malloc(2);
      card->validto[0] =
          ((container[9] & 0x0F) << 4) + ((container[10] & 0xF0) >> 4);
      card->validto[1] =
          ((container[8] & 0x0F) << 4) + ((container[9] & 0xF0) >> 4);
...
std::vector<uint8_t> EMVReader::emv_get_processing_options_no_pdol() {
  ...
  uint8_t ask_for_afl[] = {0x80, 0xa8, 0x00, 0x00,
                           0x02, 0x83, 0x00, 0x00}; // Get AFL

  if (nfc->EMVinDataExchange(ask_for_afl, sizeof(ask_for_afl), response,
                             &response_len)) {
    std::vector<uint8_t> response_vector(&response[0], &response[response_len]);
    BerTlv Tlv;
    Tlv.SetTlv(response_vector);
...
std::vector<uint8_t> EMVReader::emv_read_record(uint8_t p1, uint8_t p2) {
  ...
  if (nfc->EMVinDataExchange(read_afl, sizeof(read_afl), response,
                             &response_len)) {
...
void EMVReader::read_afl(EMVCard *card, std::vector<uint8_t> *afl) {
  for (size_t i = 0; i < afl->size(); i += 4) {
    ...
    std::vector<uint8_t> afl_content =
        emv_read_record(record_start, (sfi << 3) | 0b00000100);
    if (!afl_content.empty()) {
      BerTlv Tlv;
      Tlv.SetTlv(afl_content);
      std::vector<uint8_t> container;
      if (Tlv.GetValue("5A", &container) !=
          OK) { // Get PAN(Credit Card Number). If TLV is corrupted(happens
                // often with PN532) fallback to a workaround to find
                // information
        ...
        memcpy(card->pan, container.data(),
               container.size()); // Copy data from TLV to struct
        container.clear();
...
bool is_visa(EMVCard *card) {
  for (size_t i = 0; i < AID_DICT_SIZE; i++) {
    if (memcmp(card->aid, known_aid[i].aid, 7) == 0) {
      if (known_aid[i].vendor == EMV_VISA)
        return true;
      else
        return false;
    }
...
EMVCard EMVReader::read_emv_card() {
  ...
  if (aid.empty()) { // If we can't get AID, we can't read the card
    ...
    std::vector<uint8_t> pdol =
        emv_ask_for_pdol(&aid); // Check if card require PDOL(for example, VISA)

    if (pdol.empty()) { // No PDOL(for example Mastercard)
      std::vector<uint8_t> afl =
          emv_get_processing_options_no_pdol(); // Try to get AFL without PDOL
      ...
      if (is_visa(&res)) {
        ...
        Serial.println(
            "Cartao nao-VISA com PDOL detectado, ainda nao suportado");
...
void EMVReader::display_emv(EMVCard card) {
  ...
  if (card.parsed) {
    ...
    if (!found) {
      padprintln("Fornecedor cartao desconh.");
    }
    if (card.pan != nullptr) {
  ...
  options.emplace_back("Salvar", [this, aid, pan, issuedate, validto]() {
    this->save_emv(aid.c_str(), pan.c_str(), issuedate.c_str(),
                   validto.c_str());
  });
...
void EMVReader::save_emv(const char *aid, const char *pan,
