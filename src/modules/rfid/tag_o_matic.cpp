...
TagOMatic::TagOMatic(RFID_State initial_state) {
  if (initial_state == CLONE_MODE || initial_state == WRITE_MODE ||
      initial_state == SAVE_MODE) {
...
void TagOMatic::select_state() {
  ...
  if (_read_uid) {
    ...
    options.emplace_back("UID Customizado",
                         [this]() { set_state(CUSTOM_UID_MODE); });
...
void TagOMatic::set_state(RFID_State state) {
  ...
  case WRITE_MODE:
    if (!_rfid->pageReadSuccess)
      padprintln("[!] Blocos de dados incompletos");
...
void TagOMatic::display_banner() {
  ...
  switch (current_state) {
  case READ_MODE:
    printSubtitle("MODO LEITURA");
    break;
  case SCAN_MODE:
    printSubtitle("MODO SCAN");
    break;
  case CHECK_MODE:
    printSubtitle("MODO VERIFICAR");
    break;
  case LOAD_MODE:
    printSubtitle("MODO CARREGAR");
    break;
  case CLONE_MODE:
    printSubtitle("MODO CLONAR");
    break;
  case CUSTOM_UID_MODE:
    printSubtitle("MODO UID CUST.");
    break;
  case ERASE_MODE:
    printSubtitle("MODO APAGAR");
    break;
  case WRITE_MODE:
    printSubtitle("MODO GRAVAR DADOS");
    break;
  case WRITE_NDEF_MODE:
    printSubtitle("MODO GRAVAR NDEF");
    break;
  case SAVE_MODE:
    printSubtitle("MODO SALVAR");
    break;
  }
...
void TagOMatic::dump_check_details() {
  ...
  padprintln("UID: " +
             String(_sourceUID == _rfid->printableUID.uid ? "OK" : "NAO OK"));
  padprintln("Dados: " +
             String(_sourcePages == _rfid->strAllPages ? "OK" : "NAO OK"));
  padprintln("");
...
void TagOMatic::dump_ndef_details() {
  ...
  padprintln("Payload size: " + String(_rfid->ndefMessage.payloadSize) +
             " bytes");
}

void TagOMatic::dump_scan_results() {
  for (int i = _scanned_tags.size(); i > 0; i--) {
    if (_scanned_tags.size() > SCAN_DUMP_SIZE &&
        i <= (int)(_scanned_tags.size() - SCAN_DUMP_SIZE))
      return;
...
String TagOMatic::read_tag_headless(int timeout_seconds) {
  ...
  while ((millis() - startTime) < (uint32_t)(timeout_seconds * 1000)) {
...
int TagOMatic::write_tag_headless(int timeout_seconds) {
  if (!_rfid)
    return RFIDInterface::TAG_NOT_PRESENT;
  ...
  esp_task_wdt_config_t config = {.timeout_ms = 5000,
  ...
  while (file.available()) {
    ...
    if (line.startsWith("Device type:")) {
      ...
      for (int i = 0; i < (int)_rfid->uid.size && i < 10; i++) {
        _rfid->uid.uidByte[i] =
            strtoul(uidStr.substring(i * 2, i * 2 + 2).c_str(), NULL, 16);
      }
      ...
      atqaStr.replace(" ", "");
      if (atqaStr.length() >= 4) {
        _rfid->uid.atqaByte[0] =
            strtoul(atqaStr.substring(0, 2).c_str(), NULL, 16);
        _rfid->uid.atqaByte[1] =
            strtoul(atqaStr.substring(2, 4).c_str(), NULL, 16);
      }
