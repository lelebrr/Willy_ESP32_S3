...
void PN532KillerTools::loop() {
  while (1) {
    ...
    // If device not initialized, wait for user to press OK for hardware
    // detection
    ...
    if (_workMode == PN532KillerCmd::WorkMode::Reader) {
      ...
      if (check(NextPress)) {
        setEmulatorNextSlot(false, false);
      }
      if (check(PrevPress)) {
        setEmulatorNextSlot(true, false);
      }
    ...
    if (_tcpEnabled) {
      if (!_tcpHasClient) {
        ...
        if (newClient) {
          ...
          printCenterFootnote(String("TCP:") +
                              _tcpClient.remoteIP().toString());
        ...
        if (_tcpHasClient &&
            millis() - _tcpLastPacketMs > TCP_REMOTE_TIMEOUT_MS) {
          _tcpClient.stop();
          _tcpHasClient = false;
          printCenterFootnote("Waiting TCP client...");
        }
...
void PN532KillerTools::drainUartToUdp(bool log) {
  bool anyNet =
      (_udpEnabled && _udpHasRemote) || (_tcpEnabled && _tcpHasClient);
  if (!anyNet)
    return;
...
void PN532KillerTools::setEmulatorNextSlot(bool reverse, bool redrawTypeName) {
  ...
  if (redrawTypeName) {
    ...
    String typeName;
    switch (_tagType) {
    case PN532KillerCmd::TagType::MFC1K:
      typeName = "MFC 1K";
      break;
    case PN532KillerCmd::TagType::NTAG:
      typeName = "NTAG";
      break;
    case PN532KillerCmd::TagType::ISO15693:
      typeName = "ISO15693";
      break;
    case PN532KillerCmd::TagType::EM4100:
      typeName = "EM4100";
      break;
    default:
      typeName = "Unknown";
...
bool PN532KillerTools::enableBleDataTransfer() {
  ...
  // Optional: also set callback on TX to detect client writing wrong
  // characteristic
...
bool PN532KillerTools::enableTcpDataTransfer() {
  ...
  if (margin > 24)
    margin = 24;
  int baseY = tftHeight / 2 - 10;
  tft.fillRect(margin - 4, baseY - 4, tftWidth - margin * 2 + 8, FM * 24 + 8,
               TFT_BLACK);
  tft.setCursor(margin, baseY);
...
void PN532KillerTools::udpWifiSelectMenu() {
  ...
  selOptions.push_back({"My Network", [&]() {
                          ...
                          uint32_t t = millis();
                          while (!WiFi.isConnected() && millis() - t < 5000)
                            delay(100);
                          if (!WiFi.isConnected()) {
