...
static uint8_t hex2int(char *ch) {
  ...
  lnib = hex2digit(ch[0]);
  if (lnib == _HEX_DIGIT_ERROR)
    return _HEX_DIGIT_ERROR;

  rnib = hex2digit(ch[1]);
  if (rnib == _HEX_DIGIT_ERROR)
    return _HEX_DIGIT_ERROR;
...
void RFID125::select_state() {
  ...
  if (_tag_read) {
    ...
    //     }}); options.push_back({"Write data", [this]() {
...
void RFID125::dump_card_details() {
  padprintln("Dados Tag: " + _printable_data);
}
...
bool RFID125::write_file(String filename) {
  ...
  String file_data = "";
  for (uint8_t i = 0; i < (uint8_t)RFID125_PACKET_SIZE; i++) {
...
void RFID125::format_data() {
  _printable_data = "";
  for (uint8_t i = 1; i < (uint8_t)RFID125_PACKET_SIZE - 3; i += 2) {
