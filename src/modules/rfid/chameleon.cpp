...
void Chameleon::displayBanner() {
  ...
  switch (currentMode) {
  case BATTERY_INFO_MODE:
    printSubtitle("INFO BATERIA");
    break;
  case FACTORY_RESET_MODE:
    printSubtitle("RESET FABRICA");
    break;
  case FULL_SCAN_MODE:
    printSubtitle("SCAN COMPLETO");
    break;

  case LF_READ_MODE:
    printSubtitle("LF MODO LEITURA");
    break;
  case LF_SCAN_MODE:
    printSubtitle("LF MODO SCAN");
    break;
  case LF_CLONE_MODE:
    printSubtitle("LF MODO CLONE");
    break;
  case LF_CUSTOM_UID_MODE:
    printSubtitle("LF UID CUSTOM.");
    break;
  case LF_EMULATION_MODE:
    printSubtitle("LF MODO EMULACAO");
    break;
  case LF_SAVE_MODE:
    printSubtitle("LF MODO SALVAR");
    break;
  case LF_LOAD_MODE:
    printSubtitle("LF MODO CARREGAR");
    break;

  case HF_READ_MODE:
    printSubtitle("HF MODO LEITURA");
    break;
  case HF_SCAN_MODE:
    printSubtitle("HF MODO SCAN");
    break;
  case HF_CLONE_MODE:
    printSubtitle("HF MODO CLONE");
    break;
  case HF_WRITE_MODE:
    printSubtitle("HF MODO GRAVAR");
    break;
  case HF_CUSTOM_UID_MODE:
    printSubtitle("HF UID CUSTOM.");
    break;
  case HF_EMULATION_MODE:
    printSubtitle("HF MODO EMULACAO");
    break;
  case HF_SAVE_MODE:
    printSubtitle("HF MODO SALVAR");
    break;
  case HF_LOAD_MODE:
    printSubtitle("HF MODO CARREGAR");
    break;
  }
...
void Chameleon::dumpHFCardDetails() {
  ...
  if (!pageReadSuccess)
    padprintln("[!] Falha ao ler blocos de dados");
}

void Chameleon::dumpScanResults() {
  for (int i = (int)_scanned_tags.size(); i > 0; i--) {
    if (_scanned_tags.size() > 5 && (size_t)i <= _scanned_tags.size() - 5)
      return;
    padprintln(String(i) + ": " + _scanned_tags[i - 1].tagType + " | " +
...
bool Chameleon::isMifareClassic(byte sak) {
  return (sak == 0x08 || sak == 0x09 || sak == 0x10 || sak == 0x11 ||
          sak == 0x18 || sak == 0x19);
}
...
void Chameleon::emulateLF() {
  ...
  if (chmUltra.cmdEnableSlot(slot, chmUltra.RFID_LF) &&
...
bool Chameleon::writeFileLF(String filename) {
  ...
  if ((*fs).exists("/WillyRFID/" + filename + ".rfidlf")) {
    int i = 1;
    filename += "_";
    while ((*fs).exists("/WillyRFID/" + filename + String(i) + ".rfidlf"))
      i++;
    filename += String(i);
  }
...
void Chameleon::parseLFUID() {
  ...
  for (size_t i = 0; i < strUID.length(); i += 2) {
    lfTagData.uidByte[i / 2] =
        strtoul(strUID.substring(i, i + 2).c_str(), NULL, 16);
  }
}
...
void Chameleon::readHFTag() {
  ...
  if (!chmUltra.cmd14aScan())
    return;

  displayInfo("Lendo blocos de dados...");
  if (chmUltra.hfTagData.sak == 0x00)
    chmUltra.cmdMfuVersion();
...
void Chameleon::emulateHF() {
  ...
  if (chmUltra.cmdEnableSlot(slot, chmUltra.RFID_HF) &&
      chmUltra.cmdChangeActiveSlot(slot) &&
      chmUltra.cmdChangeSlotType(slot, tagType) &&
      chmUltra.cmdMfEload(strDump) &&
      chmUltra.cmdMfEconfig(hfTagData.uidByte, hfTagData.size,
                            hfTagData.atqaByte, hfTagData.sak) &&
      chmUltra.cmdChangeMode(chmUltra.HW_MODE_EMULATOR)) {
...
bool Chameleon::readFileHF() {
  ...
  while (file.available()) {
    ...
    if (line.startsWith("Device type:"))
      printableHFUID.piccType = strData;
    if (line.startsWith("UID:"))
      printableHFUID.uid = strData;
    if (line.startsWith("SAK:"))
      printableHFUID.sak = strData;
    if (line.startsWith("ATQA:"))
      printableHFUID.atqa = strData;
    if (line.startsWith("Pages total:"))
      dataPages = strData.toInt();
    if (line.startsWith("Pages read:"))
...
bool Chameleon::writeFileHF(String filename) {
  ...
  if ((*fs).exists("/WillyRFID/" + filename + ".rfid")) {
    int i = 1;
    filename += "_";
    while ((*fs).exists("/WillyRFID/" + filename + String(i) + ".rfid"))
      i++;
    filename += String(i);
  }
...
bool Chameleon::readMifareClassicDataBlocks(uint8_t *key) {
  // bool sectorReadSuccess; // Removed unused variable
  ...
  for (byte i = 0; i < totalPages; i++) {
    if (!chmUltra.cmdMfReadBlock(i, key))
      return false;
...
bool Chameleon::readMifareUltralightDataBlocks() {
  ...
  for (byte i = 0; i < totalPages; i++) {
    if (!chmUltra.cmdMfuReadPage(i))
      return false;
    if (chmUltra.cmdResponse.dataSize == 0)
      break;
...
bool Chameleon::writeHFDataBlocks() {
  ...
  while (strAllPages.length() > 0) {
    ...
    if (isMifareClassic(chmUltra.hfTagData.sak)) {
      ...
      if (pageIndex < 4 || pageIndex >= dataPages - 5)
        continue; // Data blocks for NTAG21X
    ...
    progressHandler(totalSize - strAllPages.length(), totalSize,
                    "Writing data blocks...");
  }
...
void Chameleon::parseHFData() {
  ...
  for (size_t i = 0; i < strATQA.length(); i += 2) {
    hfTagData.atqaByte[i / 2] =
        strtoul(strATQA.substring(i, i + 2).c_str(), NULL, 16);
  }
}
...
void Chameleon::saveScanResult() {
  ...
  if (!(*fs).exists("/WillyRFID"))
    (*fs).mkdir("/WillyRFID");
  if (!(*fs).exists("/WillyRFID/Scans"))
    (*fs).mkdir("/WillyRFID/Scans");
  if ((*fs).exists("/WillyRFID/Scans/" + filename + ".rfidscan")) {
    int i = 1;
    filename += "_";
    while (
        (*fs).exists("/WillyRFID/Scans/" + filename + String(i) + ".rfidscan"))
      i++;
    filename += String(i);
  }
  File file =
      (*fs).open("/WillyRFID/Scans/" + filename + ".rfidscan", FILE_WRITE);
