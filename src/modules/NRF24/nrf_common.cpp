...
RF24 NRFradio(willyConfigPins.NRF24_bus.io0, willyConfigPins.NRF24_bus.cs);
...
bool nrf_start(NRF24_MODE mode) {
    ...
    if (CHECK_NRF_UART(mode)) {
        ...
        NRFSerial.begin(115200, SERIAL_8N1, willyConfigPins.uart_bus.rx, willyConfigPins.uart_bus.tx);
    ...
    pinMode(willyConfigPins.NRF24_bus.io0, OUTPUT);
    digitalWrite(willyConfigPins.NRF24_bus.io0, LOW);

    if (willyConfigPins.NRF24_bus.mosi == (gpio_num_t)TFT_MOSI &&
        willyConfigPins.NRF24_bus.mosi != GPIO_NUM_NC) { // (T_EMBED), CORE2 and others
    ...
    } else if (willyConfigPins.NRF24_bus.mosi == willyConfigPins.SDCARD_bus.mosi) {
        // CC1101 shares SPI with SDCard (Cardputer and CYDs)

        NRFSPI = &sdcardSPI;
    } else if (willyConfigPins.NRF24_bus.mosi == willyConfigPins.CC1101_bus.mosi &&
               willyConfigPins.NRF24_bus.mosi != willyConfigPins.SDCARD_bus.mosi) {
    ...
    NRFSPI->begin(
        (int8_t)willyConfigPins.NRF24_bus.sck,
        (int8_t)willyConfigPins.NRF24_bus.miso,
        (int8_t)willyConfigPins.NRF24_bus.mosi
    );
    delay(10);

    if (NRFradio.begin(
            NRFSPI,
            rf24_gpio_pin_t(willyConfigPins.NRF24_bus.io0),
            rf24_gpio_pin_t(willyConfigPins.NRF24_bus.cs)
        )) {
        result = true;
    } else {
        return false;
