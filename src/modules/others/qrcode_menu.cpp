...
String calculate_crc(String input) {
  ...
  while (crc_str.length() < 4)
    crc_str = "0" + crc_str; // Pad with zeros if needed
...
void pix_qrcode() {
  ...
  String key_length =
      key.length() >= 10 ? String(key.length()) : "0" + String(key.length());
  String amount = num_keyboard("1000.00", 10, "Int amount:");
  amount = String(amount.toFloat());
  String amount_length = amount.length() >= 10 ? String(amount.length())
                                               : "0" + String(amount.length());
  ...
  String pix_code = "00020126" + String(data0.length()) + data0 +
                    ...
                    "5802BR5909Willy PIX6014Rio de Janeiro62070503***6304";
...
void qrcode_menu() {
  ...
  for (const auto &entry : willyConfig.qrCodes) {
    options.push_back(
        {entry.menuName.c_str(), lambdaHelper(qrcode_display, entry.content)});
  }
...
void custom_qrcode_menu() {
  options = {{"Display", display_custom_qrcode},
...
void save_and_display_qrcode() {
  ...
  if (std::any_of(willyConfig.qrCodes.begin(), willyConfig.qrCodes.end(),
                  [&](const WillyConfig::QrCodeEntry &entry) {
