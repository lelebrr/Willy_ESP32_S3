...
void detectInputs() {
    ...
    defined(ARDUINO_M5STICK_C_PLUS2) // check(EscPress) is the same of
                                     // check(PrevPress) in these devices
  if (check(SelPress))
#else
...
void checkCollisions() {
  for (int i = 0; i < 5; i++) {
    if ((sharkX < fish[i].x + fish[i].size) &&
        (sharkX + sharkSize > fish[i].x) &&
        (sharkY < fish[i].y + fish[i].size) &&
        (sharkY + sharkSize > fish[i].y)) {
...
void shark_loop() {
  ...
  for (;;) {
    ...
    displayScore();
    detectInputs();
    if (millis() - time > (uint32_t)downTime) {
      // Move o tubarão
      moveShark();
      // Desenha o tubarão
      drawShark();
    ...
    if (score == 99) {
      ...
      options = {
          ...
          {"IncursioHack",
           [=]() { displayInfo("github.com/IncursioHack", true); }},
