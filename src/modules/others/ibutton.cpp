...
void setup_ibutton() {
Reset:
    oneWire = new OneWire(willyConfigPins.iButton);
...
void write_byte_rw1990(byte data) {
    int data_bit;
    uint8_t pin = willyConfigPins.iButton;
...
void setiButtonPinMenu() {
    ...
    loopOptions(options, willyConfigPins.iButton + 1);
    options.clear();
    willyConfigPins.setiButtonPin(sel);
}
#endif
