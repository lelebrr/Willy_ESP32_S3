/**
 * @file timer.cpp
 * @author Aleksei Gor (https://github.com/AlekseiGor) - Reviewed and optimized
 * by Senape3000
...
void Timer::setup() {
  ...
  while (true) {
    ...
    snprintf(timeString, sizeof(timeString), "%02d:%02d:%02d", hours % 100,
             minutes % 100, seconds % 100);
    ...
    if (check(NextPress)) {
      switch (settingMode) {
      case SETTING_HOURS:
        hours = (hours >= MAX_HOURS) ? 0 : hours + 1;
        break;
      case SETTING_MINUTES:
        minutes = (minutes >= MAX_MINUTES) ? 0 : minutes + 1;
        break;
      case SETTING_SECONDS:
        seconds = (seconds >= MAX_SECONDS) ? 0 : seconds + 1;
        break;
      case SETTING_SOUND:
    ...
    if (check(PrevPress)) {
      switch (settingMode) {
      case SETTING_HOURS:
        hours = (hours <= 0) ? MAX_HOURS : hours - 1;
        break;
      case SETTING_MINUTES:
        minutes = (minutes <= 0) ? MAX_MINUTES : minutes - 1;
        break;
      case SETTING_SECONDS:
        seconds = (seconds <= 0) ? MAX_SECONDS : seconds - 1;
        break;
      case SETTING_SOUND:
        playSoundOnFinish = !playSoundOnFinish; // Toggle
        break;
  ...
  while (true) {
    ...
    if (currentMillis - lastUpdateMillis >= DISPLAY_UPDATE_INTERVAL) {
      ...
      if (seconds != lastSeconds) {
        snprintf(timeString, sizeof(timeString), "%02d:%02d:%02d", hours,
                 minutes, seconds);
