 ...
 *
 * This function is weak, meaning it can be overridden by specific board
 * implementations in main.cpp if they require custom I2C or GPIO setup for
 * their audio hardware. If no custom setup is needed, it checks for generic
 * AMP_EN pins or IO expanders.
 *
 * @param enable True to enable the speaker, false to disable.
 */
__attribute__((weak)) void _setup_codec_speaker(bool enable) {
#ifdef IO_EXP_SPKR
  if (enable)
    ioExpander.turnPinOnOff(IO_EXP_SPKR, HIGH);
  else
    ioExpander.turnPinOnOff(IO_EXP_SPKR, LOW);
#endif
#ifdef AMP_EN_PIN
  if (AMP_EN_PIN >= 0) {
...
static const uint32_t AUDIO_TASK_STACK_SIZE =
    16384; // 16KB - increased for complex MP3 files
...
struct AudioPlayerState {
  ...
  AudioPlayerState()
      : generator(nullptr), source(nullptr), output(nullptr),
        state(PLAYBACK_IDLE), mode(PLAYBACK_BLOCKING), currentFile(""),
        stopRequested(false), pauseRequested(false), volumeChanged(false),
        newVolume(0), currentGain(1.0f), startTime(0), pausedTime(0),
        totalPausedDuration(0), taskHandle(nullptr), mutex(nullptr) {
...
static void initAudioPlayer() {
  if (!g_audioPlayer) {
    g_audioPlayer = new AudioPlayerState();
  }
}
...
bool stopAudioPlayback() {
  ...
  if (g_audioPlayer->state == PLAYBACK_IDLE) {
    return false;
  }
  ...
  while (g_audioPlayer->taskHandle != nullptr &&
...
bool pauseAudioPlayback() {
  ...
  if (currentState != PLAYBACK_PLAYING && currentState != PLAYBACK_PAUSED) {
    return false;
  }
...
AudioPlaybackInfo getAudioPlaybackInfo() {
  ...
  if (g_audioPlayer->lock(pdMS_TO_TICKS(100))) {
    ...
    if (g_audioPlayer->state == PLAYBACK_PLAYING) {
      info.position = millis() - g_audioPlayer->startTime -
                      g_audioPlayer->totalPausedDuration;
    } else if (g_audioPlayer->state == PLAYBACK_PAUSED) {
      info.position = g_audioPlayer->pausedTime - g_audioPlayer->startTime -
                      g_audioPlayer->totalPausedDuration;
    }
...
static bool startAsyncPlayback(AudioGenerator *generator,
                               AudioFileSource *source, AudioOutputI2S *output,
  ...
  g_audioPlayer->currentGain =
      willyConfig.soundVolume / AUDIO_VOLUME_MAX; // Store initial gain
  ...
  BaseType_t result = xTaskCreatePinnedToCore(
      audioPlaybackTask, "AudioPlayback", AUDIO_TASK_STACK_SIZE, NULL,
      AUDIO_TASK_PRIORITY, &g_audioPlayer->taskHandle, AUDIO_TASK_CORE);
...
bool playAudioFile(FS *fs, String filepath, PlaybackMode mode) {
  ...
  if (!validateAudioFile(fs, filepath)) {
    return false;
  }
  ...
  if (filepath.endsWith(".txt") || filepath.endsWith(".rtttl"))
    ...
    generator = new AudioGeneratorAAC();
  else if (filepath.endsWith(".flac"))
    generator = new AudioGeneratorFLAC();
...
bool playAudioRTTTLString(String song, PlaybackMode mode) {
  ...
  AudioFileSource *source =
      new AudioFileSourcePROGMEM(song.c_str(), song.length());
  if (!source) {
    delete generator;
    delete audioout;
    _setup_codec_speaker(false);
...
bool tts(String text, PlaybackMode mode) {
  ...
  if (mode == PLAYBACK_ASYNC) {
    Serial.println("WARNING: TTS doesn't support async mode, using blocking");
  }
...
bool isAudioFile(String filepath) {
  return filepath.endsWith(".opus") || filepath.endsWith(".rtttl") ||
         filepath.endsWith(".txt") || filepath.endsWith(".wav") ||
...
void playTone(unsigned int frequency, unsigned long duration, short waveType) {
  ...
  AudioFileSourceFunction *file =
      new AudioFileSourceFunction(duration / 1000.0);
  if (!file) {
    delete out;
    _setup_codec_speaker(false);
    return;
  ...
  float volumeScale =
      (willyConfig.soundVolume / AUDIO_VOLUME_MAX) * AUDIO_VOLUME_SCALE;
