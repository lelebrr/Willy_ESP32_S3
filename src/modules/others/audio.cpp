 ...
 * in main.cpp if they require custom I2C or GPIO setup for their audio hardware.
 * If no custom setup is needed, it checks for generic AMP_EN pins or IO expanders.
 *
 * @param enable True to enable the speaker, false to disable.
 */
__attribute__((weak)) void _setup_codec_speaker(bool enable) {
#ifdef IO_EXP_SPKR
    if (enable) ioExpander.turnPinOnOff(IO_EXP_SPKR, HIGH);
    else ioExpander.turnPinOnOff(IO_EXP_SPKR, LOW);
#endif
#ifdef AMP_EN_PIN
    if (AMP_EN_PIN >= 0) {
