...
String extractFilename(const String &filepath) {
  ...
  if (lastDot > lastSlash) {
    name = name.substring(0, lastDot - lastSlash - 1);
  }
  return name;
}
...
void drawVectorIcon(int x, int y, int size, IconType icon, uint16_t color) {
  ...
  case ICON_PLAY: {
    ...
    tft.fillTriangle(cx - r / 2 + offset, cy - r, cx - r / 2 + offset, cy + r,
                     cx + r + offset, cy, color);
    break;
  }
  ...
  case ICON_PAUSE: {
    ...
    int barWidth = r / 1.5;
    int gap = r / 4;
    tft.fillRoundRect(cx - barWidth - gap / 2, cy - r, barWidth, r * 2, 2,
                      color);
  ...
  case ICON_PREV: {
    ...
    int offset = -r / 6;
    tft.fillTriangle(cx + offset, cy - r, cx + offset, cy + r, cx - r + offset,
                     cy, color);
    tft.fillTriangle(cx + r + offset, cy - r, cx + r + offset, cy + r,
                     cx + offset, cy, color);
    break;
  }
  ...
  case ICON_VOL: {
    ...
    int offset = -r / 4;
    tft.fillTriangle(cx - r + offset, cy, cx + offset, cy - r, cx + offset,
                     cy + r, color);
  ...
  case ICON_LOOP_ON: {
    ...
    tft.fillTriangle(cx + r - 3, cy - 3, cx + r + 3, cy - 3, cx + r, cy + 3,
                     color);
...
void drawButton(int x, int y, int size, IconType icon, bool selected,
                bool active) {
  ...
  int iconPadding = 4;
  drawVectorIcon(x + iconPadding, y + iconPadding, size - 2 * iconPadding, icon,
                 iconColor);
}
...
void drawProgressBar(int x, int y, int width, int height,
  ...
  if (duration > 0) {
    ...
    // Knob at the end
    if (fillWidth > 2) {
      tft.fillCircle(x + fillWidth, y + (height / 2), 4, TFT_WHITE);
    }
  }
}
...
bool showVolumeControl(uint8_t &currentVolume) {
  ...
  auto drawVolumeBar = [&]() {
    // Dark overlay
    tft.fillRect(BAR_X - 2, BAR_Y - 2, BAR_WIDTH + 4, BAR_HEIGHT + 4,
                 TFT_BLACK);
    ...
    // Speaker icon at top
    drawVectorIcon(BAR_X + (BAR_WIDTH - 20) / 2, BAR_Y + 10, 20, ICON_VOL,
                   TFT_WHITE);
    ...
    tft.fillRect(innerBarX, innerBarY + (innerBarH - fillH), innerBarW, fillH,
                 volColor);
  ...
  while (true) {
    ...
    if (check(EscPress)) {
      if (changed)
        setAudioPlaybackVolume(currentVolume);
      delay(200);
      return false;
    }
  }
...
void musicPlayerUI(FS *fs, const String &filepath) {
  ...
  auto drawHeader = [&]() {
    ...
    tft.setCursor((tftWidth - w) / 2,
                  (ui.HEADER_HEIGHT - (8 * ui.TEXT_SIZE_LARGE)) / 2);
    tft.print(title);
  };
  auto drawTrackInfo = [&]() {
    ...
    // Center format text in box
    String AudioFormatLabel =
        player.filename.isEmpty()
            ? "AUDIO"
            : player.filename.substring(player.filename.lastIndexOf('.') + 1)
                  .toUpperCase();
    int textWidth = AudioFormatLabel.length() *
                    12;  // textSize(2): 6 pixel base * 2 = 12 pixel per char
    ...
    if (displayText.length() > maxChars) {
      ...
      displayText = scrolled.substring(player.scrollOffset,
                                       player.scrollOffset + maxChars);

      // Scroll every 300ms
      if (millis() - player.lastUpdate > 300) {
        player.scrollOffset++;
        if (player.scrollOffset >= player.filename.length() + 3)
          player.scrollOffset = 0;
  ...
  auto drawControls = [&]() {
    ...
    // PREV button
    drawButton(x, startY, ui.BUTTON_SIZE, ICON_PREV, selectedButton == BTN_PREV,
               true);
    x += ui.BUTTON_SIZE + gap;

    // PLAY/PAUSE button
    IconType ppIcon =
        (player.isPlaying && !player.isPaused) ? ICON_PAUSE : ICON_PLAY;
    drawButton(x, startY, ui.BUTTON_SIZE, ppIcon, selectedButton == BTN_PLAY,
               true);
    x += ui.BUTTON_SIZE + gap;

    // VOLUME button
    drawButton(x, startY, ui.BUTTON_SIZE, ICON_VOL,
               selectedButton == BTN_VOLUME, true);
    x += ui.BUTTON_SIZE + gap;

    // LOOP button
    IconType loopIcon = player.loopEnabled ? ICON_LOOP_ON : ICON_LOOP_OFF;
    drawButton(x, startY, ui.BUTTON_SIZE, loopIcon, selectedButton == BTN_LOOP,
               true);
  };
  ...
  drawTrackInfo();
  drawTimers();
  drawProgressBar(ui.MARGIN_X, ui.HEADER_HEIGHT + ui.DISPLAY_HEIGHT + 20,
                  tftWidth - 2 * ui.MARGIN_X, ui.PROGRESS_HEIGHT, 0, 1);
  drawControls();
  ...
  while (true) {
    ...
    if (millis() - lastPosUpdate > 200) {
      ...
      // Scroll text logic
      if (player.filename.length() > 15)
        infoNeedsRedraw = true;
      lastPosUpdate = millis();
    }

    // Loop check
    if (player.loopEnabled && info.state == PLAYBACK_IDLE && player.isPlaying &&
        !player.isPaused) {
    ...
    if (check(SelPress)) {
      ...
      if (selectedButton == BTN_PLAY &&
          millis() - player.lastPauseAction < 300) {
    ...
    if (progressNeedsRedraw) {
      drawTimers();
      drawProgressBar(ui.MARGIN_X, ui.HEADER_HEIGHT + ui.DISPLAY_HEIGHT + 20,
                      tftWidth - 2 * ui.MARGIN_X, ui.PROGRESS_HEIGHT,
                      currentPosition, duration);
    }

    if (controlsNeedRedraw)
      drawControls();
