...
static inline void poll_user_abort() {
  // check(AnyKeyPress) is part of your codebase; this polls input and sets
  // abort flag
...
static bool parse_pcap_handshake(FS &fs, const String &path,
                                 HandshakeData &hs) {
  ...
  (void)have_m1; // tracked for diagnostics
  ...
  while (f.available() && !(have_m2 && have_m3)) {
    ...
    if (incl_len == 0 || incl_len > 256 * 1024) {
      if (incl_len > 0 && incl_len < 1000000) {
        f.seek(f.position() + incl_len);
      }
      continue;
    }
    ...
    if ((size_t)f.read(pkt, read_sz) != read_sz) {
      free(pkt);
      break;
    }

    if (read_sz < incl_len) {
      f.seek(f.position() + (incl_len - read_sz));
    }
    ...
    int msg_num = -1;
    if (ack && !mic_set && !install)
      msg_num = 1; // M1
    else if (!ack && mic_set && !install && !secure)
      msg_num = 2; // M2
    else if (ack && mic_set && install)
      msg_num = 3; // M3
    else if (!ack && mic_set && !install && secure)
      msg_num = 4; // M4
    ...
    if (msg_num == 1) {
      ...
      size_t cp_len = ((size_t)eapol_len + 4) <= sizeof(hs.eapol)
                          ? ((size_t)eapol_len + 4)
                          : sizeof(hs.eapol);
 ...
 * -4 = aborted by user
 */
static int pbkdf2_hmac_sha1(const unsigned char *password, size_t plen,
                            const unsigned char *salt, size_t slen,
                            unsigned int iterations, size_t dklen,
 ...
 */
static bool derive_ptk(const uint8_t *pmk, const uint8_t *ap_mac,
                       const uint8_t *sta_mac, const uint8_t *anonce,
...
void wifi_crack_handshake(const String &wordlist_path,
                          const String &pcap_path) {
  // reset abort flag
  ...
  padprintf("AP: %02X:%02X:%02X:%02X:%02X:%02X\n", hs.ap_mac[0], hs.ap_mac[1],
            hs.ap_mac[2], hs.ap_mac[3], hs.ap_mac[4], hs.ap_mac[5]);
  padprintln("");
  ...
  while (wf.available() && !found && !g_abortRequested) {
    ...
    // Compute PMK (may be heavy) — pbkdf2 now checks g_abortRequested
    // internally
    int pbkdf2_res = pbkdf2_hmac_sha1(
        (const unsigned char *)password, strlen(password),
        (const unsigned char *)hs.ssid, strlen(hs.ssid), 4096, 32, pmk);
    ...
    // Update UI every 10 attempts for a snappier UI (but not every single
    // attempt to avoid spamming draw)
    if ((attempts % 10) == 0) {
      uint64_t elapsed = now_us() - start_time;
      double rate = (elapsed > 0) ? (attempts * 1000000.0 / elapsed) : 0;
      double seconds = elapsed / 1000000.0;
      padprintf("\rTent: %u  %.1f/s  Tempo: %.1fs   ", (unsigned int)attempts,
                rate, seconds);

      // show a small "current candidate" snippet (trim to fit)
      String cand = line;
      if (cand.length() > 20) {
        cand = cand.substring(0, 17) + "...";
      }
  ...
  padprintf("Tempo: %.1f s (%.1f/s)\n", seconds,
            attempts / (seconds > 0 ? seconds : 1.0));
  padprintln("");

  /* ----------------- Improved result UI (tidy & minimal) ----------------- */
  if (found) {
    // Clear and redraw a fresh result screen so the password line is always
    // visible.
    ...
    // Trim long passwords so they fit on one line; keep head+tail for
    // readability.
    ...
    if (display_pw.length() > MAX_DISPLAY_LEN) {
      ...
      if (tail < 3)
        tail = 3;
      display_pw = display_pw.substring(0, head) + "..." +
                   display_pw.substring(display_pw.length() - tail);
    }
