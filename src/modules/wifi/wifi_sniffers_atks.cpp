...
void packetCounter() {
  ...
  while (attack_running) {
    ...
    if (millis() - last_update > 1000) {
      ...
      padprintln("");
      padprintln("Taxa: ~" +
                 String((uint32_t)((total_packets * 1000) / (millis() + 1))) +
                 " pkt/s");
      padprintln("");
      padprintln("ESC para sair");
      last_update = millis();
...
void pingScan() {
  ...
  for (int i = 1; i < 255 && attack_running; i++) {
    ...
    if (i % 10 == 0) {
      ...
      for (int j = std::max(0, (int)alive_hosts.size() - 5);
           j < (int)alive_hosts.size(); j++) {
...
void portScanAll() {
  ...
  // Portas comuns
  uint16_t common_ports[] = {21,   22,   23,   25,   53,   80,   110,  143,
...
void karmaAttack() {
  ...
  std::vector<String> attract_ssids = {"Starbucks",   "Airport WiFi",
                                       "Hotel Guest", "Free WiFi",
                                       "xfinitywifi", "attwifi"};
