...
void fm_banner() {
  ...
  tft.setCursor(10, 10);
  tft.drawCentreString("~== Willy Radio ==~", tftWidth / 2, 10, SMOOTH_FONT);
  delay(500);
}
...
void fm_live_run(bool reserved) {
  ...
  if (!returnToMenu and fm_station != 0) {
    fm_setup(false,
             true); // Don't know why but IT WORKS ONLY when launched 2 times...
    while (!check(EscPress) && !check(SelPress)) {
      delay(100);
    }
  }
  ...
  if (!returnToMenu) {
    ...
    while (!check(EscPress) && !check(SelPress)) {
      ...
      if (noise_level != 0) {
        ...
        tft.fillRect(0, 40, tftWidth, tftHeight, willyConfig.bgColor);
        // Draw waveform based on signal strength
        for (int i = 0; i < noise_level; i++) {
          int lineHeight =
              map(noise_level, 0, SIGNAL_STRENGTH_THRESHOLD, 0, tftHeight / 2);
          int lineX = map(i, 0, noise_level - 1, 0,
                          tftWidth - 1); // Map i to within the display width
          // Ensure drawing coordinates stay within the box bounds
          int startY = constrain(20 + tftHeight / 2 - lineHeight / 2, 20,
                                 20 + tftHeight);
          int endY = constrain(20 + tftHeight / 2 + lineHeight / 2, 20,
                               20 + tftHeight);
...
bool fm_setup(bool traffic_alert, bool silent) {
  ...
  radio.beginRDS();
  if (traffic_alert) {
    radio.setRDSstation("WillyTraffic");
    radio.setRDSbuffer("Traffic Info");
  } else {
    radio.setRDSstation("WillyRadio");
    radio.setRDSbuffer("Pwned by Willy Radio!");
  }
