...
// Constantes otimizadas para rf_send
static const char *TAG_RF_SEND = "RFSend";
constexpr uint32_t MAX_TRANSMISSION_TIME_MS = 5000; // 5 segundos timeout
constexpr size_t MAX_RF_COMMANDS_PER_FILE =
    100; // Limite de comandos por arquivo
constexpr uint32_t TRANSMISSION_DELAY_MS = 100; // Delay entre transmissões
...
void sendCustomRF() {
  ...
  if (fs == NULL) { // recent menu was selected
    if (selected_code.filepath != "")
      sendRfCommand(selected_code); // a code was selected
    return;
...
bool txSubFile(FS *fs, String filepath, bool hideDefaultUI) {
  ...
  while (databaseFile.available()) {
    ...
    if (line.startsWith("Protocol:"))
      selected_code.protocol = txt;
    if (line.startsWith("Preset:"))
      selected_code.preset = txt;
    if (line.startsWith("Frequency:"))
      selected_code.frequency = txt.toInt();
    if (line.startsWith("TE:"))
      selected_code.te = txt.toInt();
    if (line.startsWith("Bit:"))
      bitList.push_back(txt.toInt()); // selected_code.Bit = txt.toInt();
    if (line.startsWith("Bit_RAW:"))
      bitRawList.push_back(txt.toInt()); // selected_code.BitRAW = txt.toInt();
    if (line.startsWith("Key:"))
      keyList.push_back(hexStringToDecimal(
          txt.c_str())); // selected_code.key = hexStringToDecimal(txt.c_str());
  ...
  int total =
      bitList.size() + bitRawList.size() + keyList.size() + rawDataList.size() >
              0
          ? 1
          : 0;
  ...
  if (selected_code.protocol != "" && selected_code.preset != "" &&
      selected_code.frequency > 0) {
    if (!bitList.empty()) {
      uint32_t frequency = selected_code.frequency;
      String protocol = selected_code.protocol;
      String preset = selected_code.preset;
      String data = selected_code.data;
      byte modulation = 2;
      ...
      if (!initRfModule("", frequency / 1000000.0))
        return false;
      ...
      if (protocol == "RcSwitch") {
        data.replace(" ", "");
        uint64_t data_val = selected_code.key;
        int pulse = selected_code.te;
        int repeat = 10;
        RCSwitch mySwitch = RCSwitch();
        if (willyConfigPins.rfModule == CC1101_SPI_MODULE) {
          mySwitch.enableTransmit(willyConfigPins.CC1101_bus.io0);
        } else {
          mySwitch.enableTransmit(willyConfigPins.rfTx);
        }
        mySwitch.setProtocol(rcswitch_protocol_no);
        if (pulse)
      ...
      willyLogger.logRF(selected_code.key, selected_code.frequency / 1000000.0,
                        (int)selected_code.protocol.toInt(),
                        selected_code.data.c_str());
      deinitRfModule();
    }
    ...
    // RAS_Data is considered one long signal, doesn't matter the number of
    // lines it has
  ...
  Serial.printf("\nSent %d of %d signals\n", sent, total);
  if (!hideDefaultUI) {
    displayTextLine("Enviado " + String(sent) + "/" + String(total), true);
  }
...
bool txSubData(const char *data, bool hideDefaultUI) {
  struct RfCodes selected_code;
  String line;
  String txt;
  int sent = 0;
  ...
  Serial.println("Processing sub data.");
  selected_code.filepath = "buffer";
  ...
  while (end != -1) {
    line = dataStr.substring(start, end);
    if (line.endsWith("\r"))
    ...
    if (line.startsWith("TE:"))
      selected_code.te = txt.toInt();
    if (line.startsWith("Bit:"))
      bitList.push_back(txt.toInt());
    if (line.startsWith("Bit_RAW:"))
      bitRawList.push_back(txt.toInt());
    if (line.startsWith("Key:"))
      keyList.push_back(strtoull(txt.c_str(), NULL, 16));
  ...
  // Continue with the same logic as txSubFile
  uint32_t frequency = selected_code.frequency;
  String protocol = selected_code.protocol;
  String preset = selected_code.preset;
  String data_field = selected_code.data;

  byte modulation = 2; // ASK by default
  float deviation = 0;
  float rxBW = 0;
  float dataRate = 0;
  ...
  if (protocol == "RcSwitch") {
    ...
    int rcswitch_protocol_no = 1; // default
    mySwitch.setProtocol(rcswitch_protocol_no);
    if (pulse)
      mySwitch.setPulseLength(pulse);
    mySwitch.setRepeatTransmit(repeat);
    ...
    for (size_t i = 0; i < bitList.size(); i++) {
      ...
      if (!hideDefaultUI) {
        tft.setCursor(0, 20);
        tft.printf("Sending %d/%d", (int)i + 1, (int)bitList.size());
      }
      mySwitch.send(data_val, selected_code.Bit);
      sent++;
      delay(100);
    }
    ...
    for (String rawData : rawDataList) {
      ...
      data.trim();
      int index = 0;
      int buff_size = 1; // at least one element
      while (index >= 0) {
      ...
      // split data into words, convert to int, and store them in
      // transmittimings
      int startIndex = 0;
      index = 0;
      for (transmittimings_idx = 0; transmittimings_idx < (size_t)buff_size;
           transmittimings_idx++) {
        index = data.indexOf(' ', startIndex);
        if (index == -1) {
          transmittimings[transmittimings_idx] =
              data.substring(startIndex).toInt();
        } else {
          transmittimings[transmittimings_idx] =
              data.substring(startIndex, index).toInt();
        }
        startIndex = index + 1;
      }
  ...
  if (!hideDefaultUI) {
    tft.setCursor(0, 40);
    tft.printf("Sent %d signals", sent);
  }

  Serial.printf("Sent %d signals\n", sent);
...
void sendRfCommand(struct RfCodes rfcode, bool hideDefaultUI) {
  ...
  byte modulation = 2; // possible values for CC1101: 0 = 2-FSK, 1 =GFSK, 2=ASK,
                       // 3 = 4-FSK, 4 = MSK
  float deviation = 1.58;
  ...
  /*  supported flipper presets:
      ...
      FuriHalSubGhzPreset2FSKDev238Async, //< FM, deviation 2.380371 kHz,
     asynchronous FuriHalSubGhzPreset2FSKDev476Async, //< FM, deviation 47.60742
     kHz, asynchronous FuriHalSubGhzPresetMSK99_97KbAsync, //< MSK,
     deviation 47.60742 kHz, 99.97Kb/s, asynchronous
      FuriHalSubGhzPresetGFSK9_99KbAsync, //< GFSK, deviation 19.042969
     kHz, 9.996Kb/s, asynchronous FuriHalSubGhzPresetCustom, //Custom Preset
  */
  ...
  if (willyConfigPins.rfModule == CC1101_SPI_MODULE) { // CC1101 in use
    ...
    if (rxBW)
      ELECHOUSE_cc1101.setRxBW(
          rxBW); // Set the Receive Bandwidth in kHz. Value from 58.03 to
                 // 812.50. Default is 812.50 kHz.
    if (dataRate)
      ELECHOUSE_cc1101.setDRate(dataRate);
    pinMode(willyConfigPins.CC1101_bus.io0, OUTPUT);
    ELECHOUSE_cc1101.setPA(
        12); // set TxPower. The following settings are possible depending on
             // the frequency band.  (-30  -20 -15
  ...
  if (protocol == "RAW") {
    ...
    // alloc buffer for transmittimings
    int *transmittimings = (int *)calloc(
        sizeof(int), buff_size + 1); // should be smaller the data.length()
    size_t transmittimings_idx = 0;
    ...
    // transform from "00 01 02 ... FF" into "00000000 00000001 00000010 ....
    // 11111111"
  ...
  willyLogger.logRF(rfcode.key, rfcode.frequency / 1000000.0,
                    (int)rfcode.protocol.toInt(), rfcode.data.c_str());

  // digitalWrite(willyConfigPins.rfTx, LED_OFF);
  deinitRfModule();
}

void RCSwitch_send(uint64_t data, unsigned int bits, int pulse, int protocol,
                   int repeat) {
  // derived from
  // https://github.com/LSatan/SmartRC-CC1101-Driver-Lib/blob/master/examples/Rc-Switch%20examples%20cc1101/SendDemo_cc1101/SendDemo_cc1101.ino
...
// ported from
// https://github.com/sui77/rc-switch/blob/3a536a172ab752f3c7a58d831c5075ca24fd920b/RCSwitch.cpp
void RCSwitch_RAW_Bit_send(RfCodes data) {
  int nTransmitterPin = willyConfigPins.rfTx;
  if (willyConfigPins.rfModule == CC1101_SPI_MODULE) {
  ...
  for (int nRepeat = 0; nRepeat < nRepeatTransmit; nRepeat++) {
    ...
    while (currentBit >= 0) { // Starts from the end of the string until the max
...
void RCSwitch_RAW_send(int *ptrtransmittimings) {
  ...
  for (int nRepeat = 0; nRepeat < nRepeatTransmit; nRepeat++) {
    ...
    while (ptrtransmittimings[currenttiming]) { // && currenttiming <
      ...
      if (ptrtransmittimings[currenttiming] >= 0) {
        ...
        // negative value
        currentlogiclevel = false;
        ptrtransmittimings[currenttiming] =
            (-1) * ptrtransmittimings[currenttiming]; // invert sign
      }
