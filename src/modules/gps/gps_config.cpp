...
bool GPSConfig::setNavigationMode(HardwareSerial &serial,
                                  GPSDynamicModel model) {
...
bool GPSConfig::setSatelliteFilter(HardwareSerial &serial,
                                   uint8_t minSignalDb) {
...
String GPSConfig::getUpdateRateString(GPSUpdateRate rate) {
  ...
  case GPS_RATE_2HZ:
    return "2 Hz";
  case GPS_RATE_5HZ:
    return "5 Hz (Wardriving)";
  case GPS_RATE_10HZ:
    return "10 Hz (Instavel)";
  default:
    return "Desconhecido";
  }
}
...
String GPSConfig::getPowerModeString(GPSPowerMode mode) {
  switch (mode) {
  case GPS_POWER_MAX_PERFORMANCE:
    return "Max Desempenho";
  case GPS_POWER_ECO:
    return "Eco (Economia)";
  case GPS_POWER_SAVE:
    return "Power Save (Minimo)";
  default:
    return "Desconhecido";
  }
}
...
String GPSConfig::getNavigationModeString(GPSNavigationMode mode) {
  switch (mode) {
  case GPS_NAV_PORTABLE:
    return "Portatil";
  case GPS_NAV_STATIONARY:
    return "Estacionario";
  case GPS_NAV_PEDESTRIAN:
    return "Pedestre";
  case GPS_NAV_AUTOMOTIVE:
    return "Automotivo";
  case GPS_NAV_SEA:
    return "Maritimo";
  case GPS_NAV_AIRBORNE:
    return "Aereo";
  default:
    return "Desconhecido";
  }
...
void GPSConfig::calcUBXChecksum(uint8_t *data, uint16_t len, uint8_t &ck_a,
                                uint8_t &ck_b) {
  ck_a = 0;
  ck_b = 0;
...
bool GPSConfig::sendUBXCommand(HardwareSerial &serial, uint8_t class_id,
                               uint8_t msg_id, uint8_t *payload,
...
bool GPSConfig::waitForUBXAck(HardwareSerial &serial, uint8_t class_id,
                              uint8_t msg_id, uint32_t timeout_ms) {
        ...
        if (idx >= 10) {
          if (buffer[0] == 0xB5 && buffer[1] == 0x62 &&
            ...
            if (buffer[3] == 0x01 && // ACK-ACK
                buffer[6] == class_id && buffer[7] == msg_id) {
