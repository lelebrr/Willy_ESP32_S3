 ...
 * @version 0.2
 * @note Updated: 2024-08-28 by Rennan Cockles
 * (https://github.com/rennancockles)
 */
...
Wardriving::~Wardriving() {
  ...
  PPM.disableOTG();
#endif
  if (serialcmdsTaskHandle != NULL)
    vTaskResume(serialcmdsTaskHandle);
}

void Wardriving::setup() {
  if (serialcmdsTaskHandle != NULL)
    vTaskSuspend(serialcmdsTaskHandle);
  wifiNetworkCount = 0;
...
bool Wardriving::begin_gps() {
  releasePins();
  GPSserial.begin(willyConfigPins.gpsBaudrate, SERIAL_8N1,
                  willyConfigPins.gps_bus.rx, willyConfigPins.gps_bus.tx);
  ...
  if (gpsConfig.mode == GPS_MODE_ADVANCED) {
    ...
    if (gpsConfig.advancedBaudrate != (uint32_t)willyConfigPins.gpsBaudrate &&
        gpsConfig.advancedBaudrate > 0) {
      GPSserial.end();
      GPSserial.begin(gpsConfig.advancedBaudrate, SERIAL_8N1,
                      willyConfigPins.gps_bus.rx, willyConfigPins.gps_bus.tx);
    }
  }
  ...
  while (1) {
    ...
    if (GPSserial.available() > 0) {
      ...
      if (gps.location.isUpdated()) {
        ...
        if (filename == "" && gps.date.year() >= CURRENT_YEAR &&
...
void Wardriving::set_position() {
  ...
  if (initial_position_set)
    distance += gps.distanceBetween(cur_lat, cur_lng, lat, lng);
  else
...
void Wardriving::display_banner() {
  ...
  padprintln("");
  if (filename != "")
    padprintln("Arquivo: " + filename.substring(0, filename.length() - 4));
  String txt = "Enc.";
  if (scanWiFi)
    txt += " WiFi: " + String(wifiNetworkCount);
  if (scanBLE)
    txt += " BLE: " + String(bluetoothDeviceCount);
  padprint(txt);
  if (foundMACAddressCount)
    padprint(" Alerta: " + String(foundMACAddressCount));
...
void Wardriving::scanWiFiBLE() {
  ...
  if (!(*fs).exists("/WillyWardriving"))
    (*fs).mkdir("/WillyWardriving");

  bool is_new_file = false;
  if (!(*fs).exists("/WillyWardriving/" + filename))
    is_new_file = true;
  ...
  if (is_new_file) {
    file.println("WigleWifi-1.6,appRelease=v" + String(WILLY_VERSION) +
                 ",model=M5Stack GPS Unit,release=v" + String(WILLY_VERSION) +
                 ",device=ESP32 M5Stack,display=SPI TFT,board=ESP32 "
                 "M5Stack,brand=Willy,star=Sol,body=4,subBody=1");
    file.println("MAC,SSID,AuthMode,FirstSeen,Channel,Frequency,RSSI,"
  ...
  if (networksFound > 0) {
    for (int i = 0; i < networksFound; i++) {
      ...
      if (!macKeyOk || registeredMACs.find(macKey) == registeredMACs.end()) {
        ...
        if (macKeyOk)
          registeredMACs.insert(macKey); // Adds MAC to cache
        int32_t channel = WiFi.channel(i);

        char buffer[512];
        snprintf(buffer, sizeof(buffer),
                 "%s,\"%s\",[%s],%04d-%02d-%02d "
                 "%02d:%02d:%02d,%ld,%ld,%ld,%f,%f,%f,%f,,,WIFI\n",
                 macAddress.c_str(), WiFi.SSID(i).c_str(),
                 auth_mode_to_string(WiFi.encryptionType(i)).c_str(),
                 gps.date.year(), gps.date.month(), gps.date.day(),
                 gps.time.hour(), gps.time.minute(), gps.time.second(), channel,
                 channel != 14 ? 2407 + (channel * 5) : 2484, WiFi.RSSI(i),
  ...
  if (scanBLE) {
    ...
    // Bluetooth Rows
    // [BD_ADDR],[Device Name],[Capabilities],[First timestamp
    // seen],[Channel],[Frequency],
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!macKeyOk || registeredMACs.find(macKey) == registeredMACs.end()) {
        ...
        if (manufacturerId != 0) {
          snprintf(manufacturerIdStr, sizeof(manufacturerIdStr), "%04X",
                   manufacturerId);
        }
        snprintf(buffer, sizeof(buffer),
                 "%s,\"%s\",Misc [BLE],%04d-%02d-%02d "
                 "%02d:%02d:%02d,0,,%d,%f,%f,%f,%f,,%s,BLE\n",
                 address.c_str(), name.c_str(), gps.date.year(),
                 gps.date.month(), gps.date.day(), gps.time.hour(),
                 gps.time.minute(), gps.time.second(), rssi, gps.location.lat(),
                 gps.location.lng(), gps.altitude.meters(),
                 gps.hdop.hdop() * 1.0, manufacturerIdStr);
        file.print(buffer);
...
scan_summary:
  if (scanWiFi || scanBLE) {
    ...
    if (scanWiFi)
      summary += " WiFi: " + String(networksFound);
    if (scanBLE)
      summary += " BLE: " + String(bleFound);
    padprintln(summary);
  }
...
void Wardriving::enforceRegisteredMACLimit() {
  if (registeredMACs.size() < MAX_REGISTERED_MACS)
    return;
...
void Wardriving::loadAlertMACs() {
  ...
  if ((*fs).exists("/WillyWardriving/alert.txt")) {
    ...
    if (alertFile) {
      ...
      alertFile.close();
      if (alertMACs.size() > 0) {
        padprintln("Carregados " + String(alertMACs.size()) +
                   " MACs de alerta");
      }
    }
  } else {
...
void Wardriving::create_filename() {
  char timestamp[20];
  sprintf(timestamp, "%02d%02d%02d_%02d%02d%02d", gps.date.year() % 100,
          gps.date.month() % 100, gps.date.day() % 100, gps.time.hour() % 100,
          gps.time.minute() % 100, gps.time.second() % 100);
  filename = String(timestamp) + "_wardriving.csv";
}

void Wardriving::releasePins() {
  rxPinReleased = false;
  if (willyConfigPins.CC1101_bus.checkConflict(willyConfigPins.gps_bus.rx) ||
      willyConfigPins.NRF24_bus.checkConflict(willyConfigPins.gps_bus.rx) ||
#if !defined(LITE_VERSION)
      willyConfigPins.W5500_bus.checkConflict(willyConfigPins.gps_bus.rx) ||
      willyConfigPins.LoRa_bus.checkConflict(willyConfigPins.gps_bus.rx) ||
#endif
      willyConfigPins.SDCARD_bus.checkConflict(willyConfigPins.gps_bus.rx)) {
    // T-Embed CC1101 and T-Display S3 Touch ties this pin to the NRF24 CS;
    // switch it to input so the GPS UART can drive it.
    pinMode(willyConfigPins.gps_bus.rx, INPUT);
    rxPinReleased = true;
  }
}

void Wardriving::checkForAlert(const String &macAddress,
...
void Wardriving::restorePins() {
  if (rxPinReleased) {
    if (willyConfigPins.CC1101_bus.checkConflict(willyConfigPins.gps_bus.rx) ||
      ...
      pinMode(willyConfigPins.gps_bus.rx, OUTPUT);
      if (willyConfigPins.gps_bus.rx == willyConfigPins.CC1101_bus.cs ||
          willyConfigPins.gps_bus.rx == willyConfigPins.NRF24_bus.cs ||
#if !defined(LITE_VERSION)
          willyConfigPins.gps_bus.rx == willyConfigPins.W5500_bus.cs ||
          willyConfigPins.gps_bus.rx == willyConfigPins.W5500_bus.cs ||
#endif
          willyConfigPins.gps_bus.rx == willyConfigPins.SDCARD_bus.cs) {
        // If it is conflicting to an SPI CS pin, keep it HIGH
        digitalWrite(willyConfigPins.gps_bus.rx, HIGH);
      } else {
        // If it is conflicting with any other SPI pin, keep it LOW
        // Avoids CC1101 Jamming and nRF24 radio to keep enabled
        digitalWrite(willyConfigPins.gps_bus.rx, LOW);
      }
    }
    rxPinReleased = false;
  }
