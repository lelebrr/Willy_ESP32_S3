...
void interpreterHandler(void *pvParameters) {
  ...
  JSValue val = JS_Eval(ctx, script, strlen(script),
                        scriptName ? scriptName : "index.js", 0);
...
std::vector<Option> getScriptsOptionsList(String currentPath,
                                          bool saveStartupScript,
                                          int rememberedIndex) {
  ...
  while (true) {
    ...
    if (isDir) {
      ...
      opt.push_back({folderTitle.c_str(), [=]() {
                       auto subOptions =
                           getScriptsOptionsList(fullPath, saveStartupScript);
                       if (subOptions.size() > 0) {
                         ...
                         loopOptions(subOptions, MENU_TYPE_SUBMENU,
                                     displayPath.c_str());
                       }
                     }});
    } else {
      ...
      String entry_title = nameOnly.substring(
          0, nameOnly.lastIndexOf(".")); // remove the extension
      opt.push_back(
          {entry_title.c_str(), [=]() {
             if (saveStartupScript) {
               willyConfig.startupAppLuaScript =
                   fullPath; // keep config name or rename in config.h later
  ...
  if (currentPath != "" && currentPath != getScriptsFolder(fs)) {
    opt.push_back({"< Back", [=]() {
                     ...
                     auto parentOptions =
                         getScriptsOptionsList(parentPath, saveStartupScript);
                     if (parentOptions.size() > 0) {
                       ...
                       int restoreIndex = 0;
                       String currentFolderName = currentPath.substring(
                       ...
                       loopOptions(parentOptions, MENU_TYPE_SUBMENU,
                                   displayPath.c_str(), restoreIndex);
                     }
                   }});
  }
