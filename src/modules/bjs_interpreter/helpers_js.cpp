...
void js_fatal_error_handler(JSContext *ctx) {
  ...
  if (strcmp(JS_ToCString(ctx, jsvMessage, &sb), "Script exited") == 0) {
    return;
  }
...
FileParamsJS js_get_path_from_params(JSContext *ctx, JSValue *argv,
  ...
  /* existence check */
  if (checkIfexist) {
    filePath.exist = filePath.fs->exists(filePath.path);
  }
...
JSValue js_value_from_json_variant(JSContext *ctx, JsonVariantConst value) {
  if (value.isNull())
    return JS_NULL;
  if (value.is<bool>())
    return JS_NewBool(value.as<bool>());
  ...
  if (value.is<JsonArrayConst>()) {
    ...
    for (JsonVariantConst item : arr) {
      JS_SetPropertyUint32(ctx, jsArr, idx++,
                           js_value_from_json_variant(ctx, item));
    }
    return jsArr;
  }
  if (value.is<JsonObjectConst>()) {
    ...
    for (JsonPairConst kv : obj) {
      ...
      JS_SetPropertyStr(ctx, jsObj, key ? key : "",
                        js_value_from_json_variant(ctx, kv.value()));
    }
    return jsObj;
  }
  if (value.is<int64_t>())
    return JS_NewInt64(ctx, value.as<int64_t>());
  if (value.is<uint64_t>()) {
    uint64_t u = value.as<uint64_t>();
    return (u <= UINT32_MAX) ? JS_NewUint32(ctx, (uint32_t)u)
                             : JS_NewFloat64(ctx, (double)u);
  }
  if (value.is<double>())
    return JS_NewFloat64(ctx, value.as<double>());
  return JS_UNDEFINED;
}
