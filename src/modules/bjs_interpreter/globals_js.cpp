...
JSValue js_load(JSContext *ctx, JSValue *this_val, int argc, JSValue *argv) {
  ...
  size_t script_len = 0;
  char *script =
      readBigFile(fileParams.fs, fileParams.path, false, &script_len);
...
static JSTimerContextState *get_timer_state(JSContext *ctx, bool create) {
  ...
  if (JS_IsObject(ctx, holder)) {
    ...
    if (opaque)
      return (JSTimerContextState *)opaque;
  }

  if (!create)
    return NULL;

  JSTimerContextState *state =
      (JSTimerContextState *)calloc(1, sizeof(JSTimerContextState));
  if (!state)
    return NULL;

  if (!JS_IsObject(ctx, holder)) {
    holder = JS_NewObjectClassUser(ctx, JS_CLASS_TIMERS_STATE);
  }
...
JSValue js_setInterval(JSContext *ctx, JSValue *this_val, int argc,
                       JSValue *argv) {
  JSTimer *th;
  int delay, i;
  JSValue *pfunc;
...
JSValue js_clearInterval(JSContext *ctx, JSValue *this_val, int argc,
                         JSValue *argv) {
...
void run_timers(JSContext *ctx) {
  ...
  while (true) {
    ...
    for (i = 0; i < MAX_TIMERS; i++) {
      th = &state->timers[i];
      if (th->allocated) {
        if (th->main && interpreter_state != 2) {
          continue;
        }
        if (th->main && interpreter_state == 2) {
          interpreter_state = 3;
        ...
        if (delayMs <= 0) {
          ...
          // If interval callback threw, state may still be allocated; keep
          // going.
          min_delay = 0;
          break;
...
// Basic require implementation for user modules
JSValue native_require(JSContext *ctx, JSValue *this_val, int argc,
                       JSValue *argv) {
  if (argc < 1) {
    return JS_ThrowTypeError(ctx, "require() expects 1 argument");
  }
  ...
  String modulePath = String(name) + ".js";

  // Try to load from SD card first
  if (((fs::FS *)&SD)->exists(modulePath)) {
    char *script = readBigFile((fs::FS *)&SD, modulePath);
    if (script) {
      JSValue result = JS_Eval(ctx, script, strlen(script), name, 0);
      free(script);
      return result;
    }
  }
  ...
  if (((fs::FS *)&LittleFS)->exists(modulePath)) {
    char *script = readBigFile((fs::FS *)&LittleFS, modulePath);
    if (script) {
      JSValue result = JS_Eval(ctx, script, strlen(script), name, 0);
      free(script);
      return result;
  ...
  return JS_ThrowTypeError(ctx, "Module not found: %s", name);
}

JSValue native_assert(JSContext *ctx, JSValue *this_val, int argc,
                      JSValue *argv) {
  bool ok = false;
  if (argc > 0)
    ok = JS_ToBool(ctx, argv[0]);
  if (ok) {
    return JS_NewBool(1);
  }
...
JSValue native_delay(JSContext *ctx, JSValue *this_val, int argc,
                     JSValue *argv) {
...
JSValue native_random(JSContext *ctx, JSValue *this_val, int argc,
                      JSValue *argv) {
  int val = 0;
...
JSValue native_parse_int(JSContext *ctx, JSValue *this_val, int argc,
                         JSValue *argv) {
  if (argc > 0 && (JS_IsString(ctx, argv[0]) || JS_IsNumber(ctx, argv[0]) ||
                   JS_IsBool(argv[0]))) {
    double d;
...
JSValue native_to_string(JSContext *ctx, JSValue *this_val, int argc,
                         JSValue *argv) {
  if (argc > 0) {
...
JSValue native_to_hex_string(JSContext *ctx, JSValue *this_val, int argc,
                             JSValue *argv) {
  if (argc > 0 && (JS_IsString(ctx, argv[0]) || JS_IsNumber(ctx, argv[0]) ||
                   JS_IsBool(argv[0]))) {
    int value;
...
JSValue native_to_lower_case(JSContext *ctx, JSValue *this_val, int argc,
                             JSValue *argv) {
...
JSValue native_to_upper_case(JSContext *ctx, JSValue *this_val, int argc,
                             JSValue *argv) {
  if (argc > 0 && JS_IsString(ctx, argv[0])) {
    JSCStringBuf sb;
    const char *s = JS_ToCString(ctx, argv[0], &sb);
...
JSValue native_exit(JSContext *ctx, JSValue *this_val, int argc,
                    JSValue *argv) {
