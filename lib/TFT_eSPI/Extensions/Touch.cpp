...
// reliability and to correct rotation handling
...
// PATCHED: Always do beginTransaction/endTransaction unconditionally.
// The original code checked the 'locked' and 'inTransaction' flags, but these
// are NOT thread-safe — the main loop task modifies them via
// startWrite/endWrite while the touch task reads them, causing endTransaction()
// to be called without a matching beginTransaction(), which crashes FreeRTOS
...
uint8_t TFT_eSPI::getTouchRaw(uint16_t *x, uint16_t *y) {
  ...
  tmp = tmp << 5;
  tmp |= 0x1f & (spi.transfer(0x90) >>
                 3); // Read last 8 bits and start new XP conversion
...
** Description:             read validated position. Return false if not
* pressed.
***************************************************************************************/
...
uint8_t TFT_eSPI::validTouch(uint16_t *x, uint16_t *y, uint16_t threshold) {
  ...
  //  Serial.print("Sample 1 x,y = ");
  //  Serial.print(x_tmp);Serial.print(",");Serial.print(y_tmp); Serial.print(",
  //  Z = ");Serial.println(z1);
  ...
  //  Serial.print("Sample 2 x,y = ");
  //  Serial.print(x_tmp2);Serial.print(",");Serial.println(y_tmp2);
...
** Description:             read callibrated position. Return false if not
* pressed.
***************************************************************************************/
...
***************************************************************************************/
void TFT_eSPI::calibrateTouch(uint16_t *parameters, uint32_t color_fg,
  ...
  for (uint8_t i = 0; i < 4; i++) {
    ...
    fillRect(_width - size - 1, _height - size - 1, size + 1, size + 1,
             color_bg);
    ...
    case 2: // up right
      ...
      break;
    case 3: // bot right
      drawLine(_width - size - 1, _height - size - 1, _width - 1, _height - 1,
               color_fg);
      drawLine(_width - 1, _height - 1 - size, _width - 1, _height - 1,
               color_fg);
      drawLine(_width - 1 - size, _height - 1, _width - 1, _height - 1,
               color_fg);
      break;
    }
  ...
  // If the measured delta of the touch x axis is bigger than the delta of the y
  // axis, the touch and TFT axes are switched.
  ...
  // in addition, the touch screen axis could be in the opposite direction of
  // the TFT axis
  ...
  if (parameters != NULL) {
    ...
    parameters[4] = touchCalibration_rotate | (touchCalibration_invert_x << 1) |
                    (touchCalibration_invert_y << 2);
  }
}
