...
uint8_t TFT_eSPI::readByte(void) {
  ...
  RD_L;
  b = gpio_get_level(
      (gpio_num_t)TFT_D0); // Read three times to allow for bus access time
...
void TFT_eSPI::pushSwapBytePixels(const void *data_in, uint32_t len) {
  ...
  uint16_t *data = (uint16_t *)data_in;
  // ILI9488 write macro is not endianess dependant, so swap byte macro not used
  // here
  while (len--) {
    tft_Write_16(*data);
    data++;
...
void TFT_eSPI::pushPixelsDMA(uint16_t *image, uint32_t len) {
  ...
  // DMA byte count for transmit is 64Kbytes maximum, so to avoid this
  // constraint small transfers are performed using a blocking call until DMA
  // capacity is reached. User sketch can prevent blocking by managing pixel
  // count and splitting into blocks of 32768 pixels maximum. (equivalent to an
  // area of ~320 x 100 pixels)
  bool temp = _swapBytes;
  _swapBytes = false;
  while (len >
         0x4000) { // Transfer 16-bit pixels in blocks if len*2 over 65536 bytes
...
bool TFT_eSPI::initDMA(bool ctrl_cs) {
  ...
  esp_err_t ret;
  spi_bus_config_t buscfg = {.mosi_io_num = TFT_MOSI,
                             ...
                             .data7_io_num = -1,
                             .max_transfer_sz =
                                 65536, // ESP32 S3 max size is 64Kbytes
                             .flags = 0,
                             .intr_flags = 0};
