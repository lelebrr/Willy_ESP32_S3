...
#include "Processors/TFT_eSPI_STM32.c"
    ...
    defined(ARDUINO_ARCH_MBED) // Raspberry Pi Pico
...
#endif
    CS_L;
    SET_BUS_WRITE_MODE; // Some processors (e.g. ESP32) allow recycling the tx
                        // buffer when rx is not used
  }
}
...
inline void TFT_eSPI::begin_tft_read(void) {
  DMA_BUSY_CHECK; // Wait for any DMA transfer to complete before changing SPI
                  // settings
#if defined(SPI_HAS_TRANSACTION) && defined(SUPPORT_TRANSACTIONS) &&           \
    !defined(TFT_PARALLEL_8_BIT) && !defined(RP2040_PIO_INTERFACE)
  if (locked) {
...
void TFT_eSPI::setViewport(int32_t x, int32_t y, int32_t w, int32_t h,
  ...
  _vpDatum = false; // Datum is at top left corner of screen (true = top left of
                    // viewport)
  ...
  _vpH =
      height(); // Equivalent of TFT height (Nb: viewport bottom edge coord + 1)
...
TFT_eSPI::TFT_eSPI(int16_t w, int16_t h) {
  ...
  locked =
      true; // Transaction mutex lock flag to ensure begin/endTranaction pairing
...
/***************************************************************************************
** Function name:           sleep
** Description:             Allows the display to be put to sleep then awake
* again
***************************************************************************************/
...
** Description:             Set graphics origin to position x,y wrt to top left
* corner
***************************************************************************************/
...
uint8_t TFT_eSPI::readcommand8(uint8_t cmd_function, uint8_t index) {
  ...
  CS_H; // Some displays seem to need CS to be pulsed here, or is just a delay
        // needed?
  CS_L;
...
void TFT_eSPI::readRect(int32_t x, int32_t y, int32_t w, int32_t h,
  ...
  while (dh--) {
    ...
    while (lw--) {
      ...
      color = ((tft_Read_8()) << 8) | (tft_Read_8());
#elif defined(ST7735_DRIVER)
      // Read the 3 RGB bytes, colour is in LS 6 bits of the top 7 bits of each
      // byte as the TFT stores colours as 18 bits
      uint8_t r = tft_Read_8() << 1;
      uint8_t g = tft_Read_8() << 1;
      uint8_t b = tft_Read_8() << 1;
      ...
      // The 6 colour bits are in MS 6 bits of each byte but we do not include
...
void TFT_eSPI::pushMaskedImage(int32_t x, int32_t y, int32_t w, int32_t h,
  ...
  // function Each mask image line assumed to be padded to an integer number of
  // bytes & padding bits are 0
  ...
  while (h--) {
    ...
    do {
      ...
      if (setCount) {
        xp += clearCount;
        clearCount = 0;
        pushImage(x + xp, y, setCount, 1,
                  iptr + xp); // pushImage handles clipping
        if (mptr >= eptr)
          break;
        xp += setCount;
      }
...
void TFT_eSPI::readRectRGB(int32_t x0, int32_t y0, int32_t w, int32_t h,
  ...
  // Read window pixel 24-bit RGB values, buffer must be set in sketch to 3 * w
  // * h
  uint32_t len = w * h;
  while (len--) {
    ...
    *data++ = tft_Read_8();
    ...
    // The 6 colour bits are in MS 6 bits of each byte, but the ILI9488 needs an
    // extra clock pulse so bits appear shifted right 1 bit, so mask the middle
    // 6 bits then shift 1 place left
...
// Temporary solution is to include the RP2040 code here
  ...
  // For ILI9225 and RP2040 the slower Arduino SPI transfer calls were used, so
  // need to swap back to 16-bit mode
...
#elif defined(RM68120_DRIVER)
  DC_C;
  tft_Write_16(TFT_CASET + 0);
  DC_D;
  tft_Write_16(x0 >> 8);
  DC_C;
  tft_Write_16(TFT_CASET + 1);
  DC_D;
  tft_Write_16(x0 & 0xFF);
  DC_C;
  tft_Write_16(TFT_CASET + 2);
  DC_D;
  tft_Write_16(x1 >> 8);
  DC_C;
  tft_Write_16(TFT_CASET + 3);
  DC_D;
  tft_Write_16(x1 & 0xFF);
  DC_C;
  tft_Write_16(TFT_PASET + 0);
  DC_D;
  tft_Write_16(y0 >> 8);
  DC_C;
  tft_Write_16(TFT_PASET + 1);
  DC_D;
  tft_Write_16(y0 & 0xFF);
  DC_C;
  tft_Write_16(TFT_PASET + 2);
  DC_D;
  tft_Write_16(y1 >> 8);
  DC_C;
  tft_Write_16(TFT_PASET + 3);
  DC_D;
  tft_Write_16(y1 & 0xFF);

  DC_C;
  tft_Write_16(TFT_RAMWR);
  DC_D;
#else
...
// ir + 1 Angles in range 0-360 Arc foreground colour anti-aliased with
// background colour at edges anti-aliased roundEnd is optional, default is
// anti-aliased straight end Note: rounded ends extend the arc angle so can
// overlap, user sketch to manage this.
{
  inTransaction = true;
...
** Description:             Smooth graphics support function for alpha
* derivation
***************************************************************************************/
...
***************************************************************************************/
// Centre at x,y
// r = arc outer radius, ir = arc inner radius. Inclusive, so arc thickness =
// r-ir+1 Angles MUST be in range 0-360 Arc foreground fg_color anti-aliased
...
// If w and h are < radius (e.g. 0,0) a circle will be drawn with centre at
// x+r,y+r Arc foreground fg_color anti-aliased with background colour at edges
// A subset of corners can be drawn by specifying a quadrants mask. A bit set in
// the mask means draw that quadrant (all are drawn if parameter missing):
//   0x1 | 0x2
...
#ifdef GC9A01_DRIVER
      uint16_t pcol =
          fastBlend((uint8_t)(alpha * PixelAlphaGain), fg_color, bg);
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  if (!gfxFont) {
    ...
    if (textfont == 2) {
      ...
      cwidth = (cwidth + 6) / 8; // Width in whole bytes for font 2, should be +
...
int16_t TFT_eSPI::drawChar(uint16_t uniCode, int32_t x, int32_t y,
  ...
  // Font is not 2 and hence is RLE encoded
  {
    ...
    if (textcolor == textbgcolor && !clip) {
      ...
      // viewport area so use faster drawing of characters and background using
      // block write
      if (textcolor != textbgcolor && textsize == 1 && !clip) {
        setWindow(xd, yd, xd + width - 1, yd + height - 1);
