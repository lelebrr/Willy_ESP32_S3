...
public:
  ...
  using TFT_eSPI::pushColors;
  using TFT_eSPI::setAddrWindow;
  using TFT_eSPI::startWrite;

#ifdef TOUCH_CS
#if !defined(TFT_PARALLEL_8_BIT) && !defined(RP2040_PIO_INTERFACE) &&          \
    !defined(TFT_PARALLEL_16_BIT)
...
public:
  ...
  void pushToSprite(tft_sprite *dest, int32_t x, int32_t y,
                    uint32_t transparent = TFT_TRANSPARENT);
