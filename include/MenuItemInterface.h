...
public:
  ...
  virtual void drawIconImg() {
    drawImg(*willyConfig.themeFS(), willyConfig.getThemeItemImg(themePath()), 0,
            imgCenterY, true, willyConfig.theme.gifDuration, false);
  }
  ...
  void draw(float scale = 1) {
    if (rotation != willyConfigPins.rotation)
      resetCoordinates();
    if (!checkTheme()) {
      ...
      if (willyConfig.theme.label)
        drawTitle(scale); // If using .GIF, labels are draw after complete,
                          // which takes some time
      drawIconImg();
      if (willyConfig.theme.label)
        drawTitle(scale); // Makes sure to draw over the image
    }
    drawStatusBar();
  }

  void drawArrows(float scale = 1) {
    tft.fillRect(arrowAreaX, iconAreaY, arrowAreaW, iconAreaH,
                 willyConfig.bgColor);
    tft.fillRect(tftWidth - arrowAreaX - arrowAreaW, iconAreaY, arrowAreaW,
                 iconAreaH, willyConfig.bgColor);
    ...
    // Left Arrow
    tft.drawWideLine(arrowX, arrowY, arrowX + arrowSize, arrowY + arrowSize,
                     lineWidth, willyConfig.priColor, willyConfig.bgColor);
    tft.drawWideLine(arrowX, arrowY, arrowX + arrowSize, arrowY - arrowSize,
                     lineWidth, willyConfig.priColor, willyConfig.bgColor);

    // Right Arrow
    tft.drawWideLine(tftWidth - arrowX, arrowY, tftWidth - arrowX - arrowSize,
                     arrowY + arrowSize, lineWidth, willyConfig.priColor,
                     willyConfig.bgColor);
    tft.drawWideLine(tftWidth - arrowX, arrowY, tftWidth - arrowX - arrowSize,
                     arrowY - arrowSize, lineWidth, willyConfig.priColor,
                     willyConfig.bgColor);
  ...
  void drawTitle(float scale = 1) {
    ...
    tft.setTextSize(FM);
    tft.drawPixel(0, 0, 0);
    tft.fillRect(arrowAreaX, titleY, tftWidth - 2 * arrowAreaX, LH * FM,
                 willyConfig.bgColor);
    int nchars = (tftWidth - 16) / (LW * FM);
    tft.drawCentreString(getName().substring(0, nchars), iconCenterX, titleY,
                         1);
  }
...
protected:
  ...
  int iconAreaH = ((tftHeight - 2 * BORDER_PAD_Y) % 2 == 0
  ...
  void clearIconArea(void) {
    tft.fillRect(iconAreaX, iconAreaY, iconAreaW, iconAreaH,
                 willyConfig.bgColor);
  }
  void clearImgArea(void) {
    tft.fillRect(7, 27, tftWidth - 14, tftHeight - 34, willyConfig.bgColor);
  }
  void resetCoordinates(void) {
    // Recalculate Center and ared due to portrait/landscape changings
    if (tftWidth > tftHeight) {
      iconAreaH = ((tftHeight - 2 * BORDER_PAD_Y) % 2 == 0
                       ? tftHeight - 2 * BORDER_PAD_Y
                       : tftHeight - 2 * BORDER_PAD_Y + 1);
    } else {
      iconAreaH = ((tftWidth - 2 * BORDER_PAD_Y) % 2 == 0
