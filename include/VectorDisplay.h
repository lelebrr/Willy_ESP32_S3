...
public:
  ...
  void sendCommandWithAck(char c, const void *arguments, int argumentsLength) {
    ...
    do {
      ...
      while ((millis() - t0) < 500) {
        if (readMessage(NULL) && !memcmp(readBuf, "Acknwld", 7) &&
            readBuf[7] == c) {
          done = true;
          break;
        }
  ...
  void addPolyLine(int16_t x, int16_t y) {
    if (polyLineCount > 0) {
      ...
      polyLineSum +=
          args.bytes[0] + args.bytes[1] + args.bytes[2] + args.bytes[3];
      polyLineCount--;
      if (polyLineCount == 0) {
  ...
  void initialize(int w = VECTOR_DISPLAY_DEFAULT_WIDTH,
                  int h = VECTOR_DISPLAY_DEFAULT_HEIGHT) {
  ...
  void arc(int x, int y, int r, FixedPoint32 angle1, FixedPoint32 sweep,
           bool fill = false) {
    args.arc.x = x;
    args.arc.y = y;
    args.arc.r = r;
    ...
    while (remoteAvailable()) {
      ...
      if (2 <= readPos) {
        ...
        if (readPos >= VECTOR_DISPLAY_MESSAGE_SIZE) {
          ...
          if (msg->what == MESSAGE_DOWN || msg->what == MESSAGE_UP ||
              msg->what == MESSAGE_MOVE) {
  ...
  uint32_t getBitmap1Size(int16_t w, int16_t h, uint8_t flags = 0) {
    return (flags & FLAG_PAD_BYTE) ? ((uint32_t)w + 7) / 8 * h
                                   : ((uint32_t)w * h + 7) / 8;
  }

  uint32_t getBitmapSize(int16_t w, int16_t h, uint8_t depth = 1,
                         uint8_t flags = 0) {
    if (depth == 1) {
  ...
  void bitmap_progmem(int16_t x, int16_t y, const uint8_t *bmp, int16_t w,
                      int16_t h, uint8_t depth = 1, uint8_t flags = 0,
  ...
  void bitmap(int16_t x, int16_t y, uint8_t *bmp, int16_t w, int16_t h,
              uint8_t depth, uint8_t flags = 0, uint8_t *mask = NULL,
              uint32_t foreColor = 0xFFFFFFFF,
    ...
    uint8_t sum =
        sumBytes(&args, headerSize) + sumBytes((void *)bmp, bitmapSize);
    if (maskSize > 0) {
  ...
  void cp437(bool s) {
    // if true, activates real cp437 mode; if false, activates buggy Arduino
    // compatible cp437 mode
    fixCP437 = !s;
    args.attribute8.attr = 'i';
  ...
  void fillEllipse(int16_t x, int16_t y, int32_t rx, int32_t ry,
    ...
    // TODO
    if (rx < ry)
      fillCircle(x, y, rx, color);
    else
      fillCircle(x, y, ry, color);
  }

  virtual void begin(int width = VECTOR_DISPLAY_DEFAULT_WIDTH,
                     int height = VECTOR_DISPLAY_DEFAULT_HEIGHT) {
    remoteFlush();
  ...
  void drawBitmap(int16_t x, int16_t y, const uint8_t bmp[], int16_t w,
                  int16_t h, uint16_t color) /* PROGMEM */ {
    bitmap_progmem(x, y, bmp, w, h, 1, 0, NULL, color565To8888(color),
                   0); // transparent background
  }

  void drawBitmap(int16_t x, int16_t y, uint8_t *bmp, int16_t w, int16_t h,
                  uint16_t color) {
    bitmap(x, y, bmp, w, h, 1, 0, NULL, color565To8888(color),
           0); // transparent background
  }
  ...
  void drawXBitmap(int16_t x, int16_t y, const uint8_t bmp[], int16_t w,
                   int16_t h, uint16_t color) {
    bitmap_progmem(x, y, bmp, w, h, 1, FLAG_PAD_BYTE | FLAG_LOW_ENDIAN_BITS,
                   NULL, color565To8888(color), 0);
  }

  void drawXBitmap(int16_t x, int16_t y, const uint8_t bmp[], int16_t w,
                   int16_t h, uint16_t color, uint16_t bgcolor) {
    bitmap_progmem(x, y, bmp, w, h, 1, FLAG_PAD_BYTE | FLAG_LOW_ENDIAN_BITS,
                   NULL, color565To8888(color), color565To8888(bgcolor));
  }

  void drawGrayscaleBitmap(int16_t x, int16_t y, const uint8_t bmp[], int16_t w,
                           int16_t h) {
    bitmap_progmem(x, y, bmp, w, h, 8, 0, NULL);
  }

  void drawGrayscaleBitmap(int16_t x, int16_t y, uint8_t *bmp, int16_t w,
                           int16_t h) {
  ...
  void drawGrayscaleBitmap(int16_t x, int16_t y, uint8_t *bmp, uint8_t *mask,
                           int16_t w, int16_t h) {
    bitmap(x, y, bmp, w, h, 8, 0, mask);
  }

  void drawRGBBitmap(int16_t x, int16_t y, uint16_t *bmp, int16_t w,
                     int16_t h) {
    bitmap(x, y, (uint8_t *)bmp, w, h, 16, FLAG_LOW_ENDIAN_BYTES, NULL);
  }

  void drawRGBBitmap(int16_t x, int16_t y, const uint16_t bmp[], int16_t w,
                     int16_t h) {
    bitmap_progmem(x, y, (const uint8_t *)bmp, w, h, 16, FLAG_LOW_ENDIAN_BYTES,
                   NULL);
  }
  ...
  void drawRGBBitmap(int16_t x, int16_t y, uint16_t *bmp, uint8_t *mask,
                     int16_t w, int16_t h) {
  ...
  void fillCircleHelper(int16_t cx, int16_t cy, int16_t r, uint8_t corners,
    ...
    if (corners & 1) {
      arc(cx, cy, r, TO_FP32(270), TO_FP32(180),
          false); // drawing edges separately makes things fit better
...
#pragma GCC diagnostic ignored "-Wunused-parameter"
  ...
  void getTextBounds(const void /*__FlashStringHelper*/ *s, int16_t x,
...
public:
  ...
  void begin(uint32_t speed, int width = VECTOR_DISPLAY_DEFAULT_WIDTH,
             int height = VECTOR_DISPLAY_DEFAULT_HEIGHT) {
#ifndef NO_SERIAL
    if (doSerialBegin) {
      Serial.begin(speed);
  ...
  virtual void begin(int width = VECTOR_DISPLAY_DEFAULT_WIDTH,
                     int height = VECTOR_DISPLAY_DEFAULT_HEIGHT) override {
...
#ifndef NO_SERIAL
  SerialDisplayClass(int16_t w = TFT_WIDTH, int16_t h = TFT_HEIGHT)
      : s(Serial), doSerialBegin(true) {}
#endif
...
public:
  bool begin(const char *host, int width = VECTOR_DISPLAY_DEFAULT_WIDTH,
             int height = VECTOR_DISPLAY_DEFAULT_HEIGHT) {
    VectorDisplayClass::begin(width, height);
